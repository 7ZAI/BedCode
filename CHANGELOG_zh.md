...
#### agent-hub 插件 — 供应商中心凭据库
- 供应商预设新增中心凭据列（`provider_preset.api_key`）：一处配置 key、分发到多个 agent（claude / pi / opencode）；列表/状态/导入结果一律掩码（前 3 字符 + 长度）、日志只记长度——明文只存插件库（2026-09-14 用户决策删除「key 不落 hub 存储」红线）
- 反向导入把源 CLI key 一并收进中心凭据库（不再只存掩码）；应用面板新增第四种 key 模式 `stored`（中心库，预设已有 key 时默认）；预设编辑器可直接设置/清空已存 key

#### 桌面端插件管理 — zip 加载与卸载
- 插件详情页对**所有来源**的插件都提供卸载（内置随包 / 文件扫描 / zip 安装）；卸载要求插件处于**未启用**状态（运行中按钮禁用并提示先停用），且清空插件全部数据：安装目录（取自 `extension_path`，含同目录私有 `plugin.db`）、键值存储与文件授权、持久化激活状态、数据库缓存连接与运行时限频记录。内置插件位于随包资源目录：只读安装下删除会如实报错，下次构建/更新后随包副本会重新出现
