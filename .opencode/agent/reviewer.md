...
Bash 仅用于只读命令：`git diff`、`git log`、`git show`、`cargo check --message-format=short`、`codegraph explore/callers/impact`。**不得修改文件、不得运行构建或测试写操作**。

定位改动的影响面优先用 codegraph MCP 工具 `codegraph_explore`（或 bash 执行 `codegraph explore "<问题>"`）。
