...
1. 项目根目录有 `.codegraph/` 索引，优先使用 codegraph MCP 工具 `codegraph_explore`：一次调用返回按文件分组的带行号源码 + 调用链 + 影响面，通常一次调用即可
2. codegraph 不可用或未覆盖时（字符串、注释、配置文本等字面量），用 bash 兜底：`codegraph explore "<问题>"`、`codegraph query <符号>`、`codegraph callers/callees/impact <符号>`；再用 grep/read 补细节
3. 识别类型定义、接口、关键函数、Tauri command 签名
4. 记录文件之间的依赖关系与调用链

彻底程度按任务推断（quick / medium / thorough），默认 medium。只读结构性问题默认最多 2 次 codegraph 调用，信任其结果，不要用 grep 重复验证。
