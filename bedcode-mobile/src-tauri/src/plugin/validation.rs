...
//!   卸载、文件服务路径全部依赖「目录名 = id」约定，
//!   不一致说明目录被伪造或复制，一律拒绝加载
//!
//! 信任模型：插件身份 = manifest id 自报字符串（无签名链），校验规则
//! 保证 id 不可歧义（一个 id 只对应一个目录、一份 manifest），为审批
//! 门禁（approval.rs）提供可钉扎的身份锚点。完整模型见
//! docs/implementation-plans/mobile-plugin-trust-hardening.md。
...
///
/// 目录名是插件文件系统的物理锚点（卸载按 `plugins_dir/{id}` 删除、
/// 文件服务挂载路径含 id），manifest.id 是权限/存储/注册表的逻辑锚点。
/// 两者不一致 = 目录被复制改名或 manifest 被替换，直接拒绝。
...
mod tests {
    ...
    fn test_invalid_ids() {
        for id in [
            ...
            ".com.bedcode",
            "Com.BedCode.X",
            "com.bedcode.my_plugin",
            "com.bedcode.-x",
            "com.bedcode.x-",
            "com.bedcode.x y",
            "com.bedcode.x/y",
            &"a".repeat(PLUGIN_ID_MAX_LEN + 1),
        ] {
