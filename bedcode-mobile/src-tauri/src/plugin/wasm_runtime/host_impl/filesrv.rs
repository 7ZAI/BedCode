...
pub(crate) fn filesrv_mount(state: &WasmPluginState, opts_json: &str) -> Result<String, String> {
    if !state
        .granted_permissions
    ...
    let options: bedcode_plugin_api_mobile::MountOptions =
        serde_json::from_str(opts_json).map_err(|e| format!("invalid MountOptions JSON: {}", e))?;
    ...
    let mount_entry = guarded_host_call(
        ...
        || {
            tokio::task::block_in_place(|| {
                state.runtime_handle.block_on(fs.registry.mount(
                    &state.plugin_id,
                    options,
    ...
    let result_json = serde_json::to_string(&result).map_err(|e| format!("serialize MountResult failed: {}", e))?;
    ...
    guarded_host_call(
        ...
        || {
            tokio::task::block_in_place(|| {
                state
                    .runtime_handle
                    .block_on(fs.registry.unmount(&state.plugin_id, mount_path))
            })
        },
    )
...
pub(crate) fn filesrv_update_roots(state: &WasmPluginState, mount_path: &str, roots_json: &str) -> Result<(), String> {
    if !state
        .granted_permissions
        .contains(bedcode_plugin_api_mobile::permission::PERMISSION_FILESERVICE)
    {
    ...
    let roots: Vec<String> = serde_json::from_str(roots_json).map_err(|e| format!("invalid roots JSON: {}", e))?;
    ...
    guarded_host_call(
        ...
        Err(crate::AppError::Internal(
            "host_filesrv_update_roots panicked".to_string(),
        )),
        || {
            tokio::task::block_in_place(|| {
                state
    ...
    crate::system::error_boundary::spawn_with_error_boundary("filesrv_wasm_after_update_roots", async move {
        fs.after_mount_changed().await;
    });
    Ok(())
}
...
pub(crate) fn filesrv_get_peer(state: &WasmPluginState, peer_id: &str) -> Result<Option<String>, String> {
    ...
    let peer = guarded_host_call(&state.plugin_id, "host_filesrv_get_peer", None, || {
        tokio::task::block_in_place(|| state.runtime_handle.block_on(fs.registry.get_peer(peer_id)))
    });
...
pub(crate) fn filesrv_query_peer(state: &WasmPluginState, peer_id: &str) -> Result<(), String> {
    ...
    guarded_host_call(
        ...
        Err(crate::AppError::WebSocket(
            "host_filesrv_query_peer panicked".to_string(),
        )),
        || {
            tokio::task::block_in_place(|| {
...
pub(crate) fn transfer_start(state: &WasmPluginState, req_json: &str) -> Result<String, String> {
    if !state
        .granted_permissions
    ...
    let request: bedcode_plugin_api_mobile::TransferRequest =
        serde_json::from_str(req_json).map_err(|e| format!("invalid TransferRequest JSON: {}", e))?;
    ...
    let authorized = guarded_host_call(&state.plugin_id, "host_transfer_start", false, || {
        tokio::task::block_in_place(|| {
            state
                .runtime_handle
                .block_on(crate::plugin::transfer::check_local_path_authorized(
                    &state.plugin_id,
                    &request,
                ))
        })
    ...
    if !authorized {
        return Err(format!("local path not authorized by user: {}", request.local_path));
    }
    ...
    let task_id = crate::plugin::transfer::spawn_transfer(request, app_handle, state.host_ctx.message_bus.clone());
    tracing::info!(
        plugin_id = %state.plugin_id,
        task_id = %task_id,
...
pub(crate) fn filesrv_approve_transfer(state: &WasmPluginState, batch_id: &str) -> Result<(), String> {
    ...
    guarded_host_call(
        ...
        Err(crate::AppError::Internal(
            "host_filesrv_approve_transfer panicked".to_string(),
        )),
        || {
            tokio::task::block_in_place(|| {
                state
...
pub(crate) fn filesrv_reject_transfer(state: &WasmPluginState, batch_id: &str) -> Result<(), String> {
    ...
    guarded_host_call(
        ...
        Err(crate::AppError::Internal(
            "host_filesrv_reject_transfer panicked".to_string(),
        )),
        || {
            tokio::task::block_in_place(|| {
                state
...
pub(crate) fn filesrv_set_approval_timeout(
    ...
    guarded_host_call(
        ...
        Err(crate::AppError::Internal(
            "host_filesrv_set_approval_timeout panicked".to_string(),
        )),
        || {
            tokio::task::block_in_place(|| {
                state
                    .runtime_handle
                    .block_on(fs.registry.set_approval_timeout(&state.plugin_id, mount_path, seconds))
                    .map_err(crate::file_service::registry::BatchError::into_app_error)
            })
        },
    )
...
pub(crate) fn filesrv_cancel_receiving(state: &WasmPluginState, session_id: &str) -> Result<(), String> {
    ...
    guarded_host_call(
        ...
        Err(crate::AppError::Internal(
            "host_filesrv_cancel_receiving panicked".to_string(),
        )),
        || {
            tokio::task::block_in_place(|| {
                state
...
pub(crate) fn filesrv_download(state: &WasmPluginState, req_json: &str) -> Result<String, String> {
    if !state
        .granted_permissions
        .contains(bedcode_plugin_api_mobile::permission::PERMISSION_FILESERVICE)
    {
    ...
    let req: bedcode_plugin_api_mobile::FileTransferRequest =
        serde_json::from_str(req_json).map_err(|e| format!("invalid FileTransferRequest JSON: {}", e))?;
    ...
    let (base, auth, dest_path) = guarded_host_call(
        ...
        || {
            tokio::task::block_in_place(|| {
                state.runtime_handle.block_on(async {
                    let (base, auth) = crate::file_service::client::desktop_http_endpoint().await?;
                    ...
                    let dest = match req.dest_path.clone() {
                        ...
                        None => {
                            let dir = fs
                                ...
                                .downloads_dir()
                                .await
                                .ok_or_else(|| "file service client: downloads dir unavailable".to_string())?;
    ...
    crate::system::error_boundary::spawn_with_error_boundary("filesrv_host_download", async move {
        ...
        let result = crate::file_service::client::download_with_retry(&client, &dreq, store, &handle, 3).await;
        ...
        let state = match result {
            ...
            Err(_) if handle.is_cancelled() => bedcode_plugin_api_mobile::TransferState::Cancelled,
...
pub(crate) fn filesrv_upload(state: &WasmPluginState, req_json: &str) -> Result<String, String> {
    if !state
        .granted_permissions
        .contains(bedcode_plugin_api_mobile::permission::PERMISSION_FILESERVICE)
    {
    ...
    let source_path = req.source_path.clone().unwrap_or_else(|| req.relative_path.clone());

    let handle = crate::file_service::client::TransferHandle::new(task_id.clone());
    crate::plugin::transfer::register_cancel_token(&task_id, handle.token());
    let saf = guarded_host_call(&state.plugin_id, "host_filesrv_upload", None, || {
        tokio::task::block_in_place(|| {
            state.runtime_handle.block_on(async {
                let fs = crate::state::get_file_service();
    ...
    crate::system::error_boundary::spawn_with_error_boundary("filesrv_host_upload", async move {
        let reporter_stop = handle.token().child_token();
        {
            let app = app_for_task.clone();
            let bus = bus_for_task.clone();
...
fn spawn_report_progress(
    ...
    crate::system::error_boundary::spawn_with_error_boundary("filesrv_host_progress", async move {
...
pub(crate) fn filesrv_respond_intent(state: &WasmPluginState, intent_id: &str, decision: &str) -> Result<(), String> {
    if !state
        .granted_permissions
        .contains(bedcode_plugin_api_mobile::permission::PERMISSION_FILESERVICE)
    {
    ...
    match decision {
        "accepted" => guarded_host_call(
            ...
            || tokio::task::block_in_place(|| responder.approve_intent(intent_id).map_err(|e| e.to_string())),
        ),
        "rejected" => guarded_host_call(
            &state.plugin_id,
            "host_filesrv_respond_intent",
            Err("host_filesrv_respond_intent panicked".to_string()),
            || tokio::task::block_in_place(|| responder.reject_intent(intent_id).map_err(|e| e.to_string())),
        ),
