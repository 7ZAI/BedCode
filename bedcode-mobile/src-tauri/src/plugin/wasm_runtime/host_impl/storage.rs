...
pub(crate) fn storage_get(state: &WasmPluginState, key: &str) -> Result<Option<String>, String> {
    ...
    let value = guarded_host_call(
        &state.plugin_id,
        "host_storage_get",
        Err(crate::AppError::Internal("host_storage_get panicked".to_string())),
        || tokio::task::block_in_place(|| state.runtime_handle.block_on(storage.get(&state.plugin_id, key))),
    )
    ...
    guarded_host_call(
        ...
        || {
            tokio::task::block_in_place(|| {
                state
                    .runtime_handle
                    .block_on(storage.set(&state.plugin_id, key, json_value))
            })
        },
    )
    .map_err(|e| format!("storage error: {}", e))
...
pub(crate) fn storage_delete(state: &WasmPluginState, key: &str) -> Result<(), String> {
    ...
    guarded_host_call(
        ...
        || tokio::task::block_in_place(|| state.runtime_handle.block_on(storage.delete(&state.plugin_id, key))),
    )
    .map_err(|e| format!("storage error: {}", e))
}
