...
pub async fn biometric_generate_keypair(fingerprint: &str) -> crate::Result<String> {
    let handle = BIOMETRIC_KEY_HANDLE
        .get()
        .ok_or_else(|| crate::AppError::Plugin("BiometricKeyPlugin not registered".to_string()))?;
    ...
    if response.get("success").and_then(|v| v.as_bool()).unwrap_or(true) == false {
        ...
        return Err(crate::AppError::Plugin(format!(
            "Biometric key generation failed: {}",
            reason
        )));
    }
    response
...
pub async fn biometric_device_supported() -> crate::Result<(bool, i32)> {
    ...
    let supported = response.get("supported").and_then(|v| v.as_bool()).unwrap_or(false);
    let reason = response.get("reason").and_then(|v| v.as_i64()).unwrap_or(-1) as i32;
    Ok((supported, reason))
}
...
pub async fn biometric_generate_keypair(_fingerprint: &str) -> crate::Result<String> {
    Err(crate::AppError::Plugin(
        "Biometric key unavailable on this platform".to_string(),
    ))
}
