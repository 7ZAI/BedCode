...
pub async fn saf_list_tree(tree_uri: &str, document_id: &str) -> crate::Result<Vec<crate::plugin::saf_io::SafEntry>> {
    let handle = super::picker::SAF_TRANSFER_HANDLE
        .get()
        .ok_or_else(|| crate::AppError::Plugin("SafTransferPlugin not registered".to_string()))?;
    let response: serde_json::Value = handle
        ...
        .await
        .map_err(|e| crate::AppError::Plugin(format!("Failed to invoke listTreeChildren: {}", e)))?;
    crate::plugin::saf_io::parse_saf_entries(&response)
}

/// 启动中转复制（SAF 源 → app 私有 cache）
#[cfg(target_os = "android")]
pub async fn saf_copy_start(uri: &str, dest_name: &str) -> crate::Result<crate::plugin::saf_io::SafCopyHandle> {
    let handle = super::picker::SAF_TRANSFER_HANDLE
        .get()
        .ok_or_else(|| crate::AppError::Plugin("SafTransferPlugin not registered".to_string()))?;
    let response: serde_json::Value = handle
        .run_mobile_plugin_async("safToCache", serde_json::json!({ "uri": uri, "destName": dest_name }))
        .await
...
/// 轮询中转复制进度
#[cfg(target_os = "android")]
pub async fn saf_copy_status(copy_id: &str) -> crate::Result<crate::plugin::saf_io::SafCopyStatus> {
    let handle = super::picker::SAF_TRANSFER_HANDLE
        .get()
        .ok_or_else(|| crate::AppError::Plugin("SafTransferPlugin not registered".to_string()))?;
    let response: serde_json::Value = handle
...
pub async fn saf_cleanup_stale_copies() -> crate::Result<()> {
    ...
    handle
        ...
        .await
        .map_err(|e| crate::AppError::Plugin(format!("Failed to invoke cleanupStaleCopies: {}", e)))
}
...
pub async fn saf_check_authorized(tree_uri: &str) -> crate::Result<bool> {
    let handle = super::picker::SAF_TRANSFER_HANDLE
        ...
        .ok_or_else(|| crate::AppError::Plugin("SafTransferPlugin not registered".to_string()))?;
    let response: serde_json::Value = handle
        .run_mobile_plugin_async("checkAuthorized", serde_json::json!({ "treeUri": tree_uri }))
        .await
...
#[cfg(target_os = "android")]
pub async fn saf_write_media_downloads(src_path: &str, display_name: &str, mime_type: &str) -> crate::Result<()> {
    let handle = super::picker::SAF_TRANSFER_HANDLE
        .get()
        .ok_or_else(|| crate::AppError::Plugin("SafTransferPlugin not registered".to_string()))?;
    let response: serde_json::Value = handle
        ...
        .await
        .map_err(|e| crate::AppError::Plugin(format!("Failed to invoke writeMediaDownloads: {}", e)))?;
    crate::plugin::saf_io::parse_media_write_response(&response)
}

/// 打开 SAF 源为可流读句柄（M3 上传流直传；offset 语义见 SafIo::open_stream）
#[cfg(target_os = "android")]
pub async fn saf_stream_open(uri: &str, offset: u64) -> crate::Result<crate::plugin::saf_io::SafStreamHandle> {
    let handle = super::picker::SAF_TRANSFER_HANDLE
        .get()
        .ok_or_else(|| crate::AppError::Plugin("SafTransferPlugin not registered".to_string()))?;
    let response: serde_json::Value = handle
...
pub async fn saf_stream_read(handle_id: &str, len: usize) -> crate::Result<Vec<u8>> {
    let handle = super::picker::SAF_TRANSFER_HANDLE
        ...
        .ok_or_else(|| crate::AppError::Plugin("SafTransferPlugin not registered".to_string()))?;
    let response: serde_json::Value = handle
        .run_mobile_plugin_async("safRead", serde_json::json!({ "handleId": handle_id, "len": len }))
        .await
...
#[cfg(target_os = "android")]
pub async fn saf_save_to_document(src_path: &str, suggested_name: &str, mime_type: &str) -> crate::Result<()> {
    let handle = super::picker::SAF_TRANSFER_HANDLE
        .get()
        .ok_or_else(|| crate::AppError::Plugin("SafTransferPlugin not registered".to_string()))?;
    let response: serde_json::Value = handle
...
#[cfg(not(target_os = "android"))]
pub async fn saf_list_tree(_tree_uri: &str, _document_id: &str) -> crate::Result<Vec<crate::plugin::saf_io::SafEntry>> {
    Err(crate::AppError::Plugin(
        "SAF storage is not available on this platform".to_string(),
    ))
}
...
#[cfg(not(target_os = "android"))]
pub async fn saf_copy_start(_uri: &str, _dest_name: &str) -> crate::Result<crate::plugin::saf_io::SafCopyHandle> {
    Err(crate::AppError::Plugin(
        "SAF storage is not available on this platform".to_string(),
    ))
}
...
#[cfg(not(target_os = "android"))]
pub async fn saf_copy_status(_copy_id: &str) -> crate::Result<crate::plugin::saf_io::SafCopyStatus> {
    Err(crate::AppError::Plugin(
        "SAF storage is not available on this platform".to_string(),
    ))
}
...
#[cfg(not(target_os = "android"))]
pub async fn saf_write_media_downloads(_src_path: &str, _display_name: &str, _mime_type: &str) -> crate::Result<()> {
    Err(crate::AppError::Plugin(
        "SAF storage is not available on this platform".to_string(),
    ))
}
...
#[cfg(not(target_os = "android"))]
pub async fn saf_stream_open(_uri: &str, _offset: u64) -> crate::Result<crate::plugin::saf_io::SafStreamHandle> {
    Err(crate::AppError::Plugin(
        "SAF storage is not available on this platform".to_string(),
    ))
}
...
#[cfg(not(target_os = "android"))]
pub async fn saf_save_to_document(_src_path: &str, _suggested_name: &str, _mime_type: &str) -> crate::Result<()> {
    Err(crate::AppError::Plugin(
        "SAF storage is not available on this platform".to_string(),
    ))
}
