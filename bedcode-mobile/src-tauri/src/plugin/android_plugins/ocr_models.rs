...
pub fn ocr_model_extractor_plugin() -> tauri::plugin::TauriPlugin<tauri::Wry> {
    Builder::new("ocr-model-extractor")
        .setup(|_app, api| {
            ...
            {
                let handle =
                    api.register_android_plugin("com.bedcode.mobile", "OcrModelExtractorPlugin")?;
...
pub async fn extract_ocr_models(app_version: &str) -> crate::Result<u32> {
    ...
    let response: serde_json::Value = handle
        ...
        .await
        .map_err(|e| {
            crate::AppError::Plugin(format!("Failed to invoke extractOcrModels: {}", e))
        })?;
...
pub async fn native_library_dir() -> crate::Result<String> {
    ...
    let response: serde_json::Value = handle
        ...
        .await
        .map_err(|e| {
            crate::AppError::Plugin(format!("Failed to invoke getNativeLibraryDir: {}", e))
        })?;
    response
        .get("dir")
        .and_then(|d| d.as_str())
        .map(|s| s.to_string())
        .ok_or_else(|| {
            crate::AppError::Plugin("getNativeLibraryDir: missing 'dir' in response".to_string())
        })
}
