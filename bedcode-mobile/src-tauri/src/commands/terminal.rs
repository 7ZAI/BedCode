...
use crate::connection::request::{SessionRequest, TerminalRequest};
...
pub async fn ws_send_input_async(
    ...
    conn.send_and_wait_with_disconnect_handling(&app_handle, &message, timeout)
        .await?;

    // 通知插件终端输入（只读通知）
    {
        let pm = crate::state::get_plugin_manager();
        pm.dispatch_lifecycle_event(crate::plugin::types::PluginLifecycleEvent::TerminalInput {
...
/// 通用接口，接受 JSON 格式的消息
#[tauri::command]
pub async fn ws_send_message(app_handle: AppHandle, message_type: String, payload: serde_json::Value) -> Result<()> {
    let conn = get_connection_manager();
    ...
    match result {
        ...
        None => Err(crate::AppError::Parse(format!(
...
pub async fn ws_send_and_wait(
    ...
    let conn = get_connection_manager();
    let timeout =
        std::time::Duration::from_secs(timeout_secs.unwrap_or(crate::connection::request::timeouts::DEFAULT.as_secs()));

    // 尝试将 payload 转换为对应的 Message 类型
    let message = match convert_json_to_message(&message_type, payload) {
        Some(m) => m,
        None => {
            return Err(crate::AppError::Parse(format!(
                "Unsupported message type: {}",
                message_type
            )))
        }
    ...
    let response = conn
        .send_and_wait_with_disconnect_handling(&app_handle, &message, timeout)
        .await?;
    let json_str = response.to_json()?;
    let parsed: serde_json::Value =
        serde_json::from_str(&json_str).map_err(|e| crate::AppError::Parse(e.to_string()))?;
...
#[tauri::command]
pub async fn ws_resize_terminal(app_handle: AppHandle, session_id: String, cols: u32, rows: u32) -> Result<()> {
    tracing::debug!(
        "[ws_resize_terminal] session_id={}, cols={}, rows={}",
        session_id,
        cols,
        rows
    );
