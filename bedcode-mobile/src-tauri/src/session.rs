...
use crate::connection::request::{timeouts, ResponseParser, SessionRequest};
use crate::Result;
...
impl SessionManager {
    ...
    pub async fn start_session(&self, config_id: &str, session_name: Option<&str>) -> Result<String> {
        ...
        Err(crate::AppError::WebSocket(
            "Failed to start session: invalid response".to_string(),
        ))
    }
    ...
    pub async fn stop_session(&self, session_id: &str) -> Result<()> {
        tracing::info!(
            "[stop_session] Sending StopSession request for session_id={}",
            session_id
        );
    ...
    pub async fn remove_session(&self, session_id: &str) -> Result<()> {
        ...
        tracing::info!(
            "[remove_session] send_and_wait result: {:?}",
