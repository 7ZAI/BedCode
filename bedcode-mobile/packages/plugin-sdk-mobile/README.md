...
| 子路径 | 内容 |
|--------|------|
| `@bedcode/plugin-sdk-mobile` | 主 API：类型 + 运行时代理 |
| `@bedcode/plugin-sdk-mobile/vite` | `bedcodePlugin()` Vite 构建插件 |
| `@bedcode/plugin-sdk-mobile/types` | 纯类型导出（仅类型，无运行时） |
| `@bedcode/plugin-sdk-mobile/ui` | 共享 Vue 组件 |
