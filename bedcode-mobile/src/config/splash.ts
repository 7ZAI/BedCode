...
/**
 * 开屏候选页注册表:每个候选对应一个前端开屏组件,
 * App.vue 依据 ACTIVE_SPLASH_CANDIDATE 决定渲染哪一个。
 * 新增候选页:在本表登记 id → 组件映射,并在 App.vue 的渲染分支补一条。
 */
export const SPLASH_CANDIDATES = {
  /** 候选 1:终端开机自检叙事动画(components/SplashScreen.vue) */
  terminal: 'terminal',
  /** 候选 2:原生 Android 系统开屏样式复刻(components/SplashScreenNative.vue) */
  native: 'native',
} as const

export type SplashCandidate = keyof typeof SPLASH_CANDIDATES

/**
 * 当前启用的开屏候选页 —— 切换开屏样式只需改这一处:
 * - 'terminal':终端开机自检叙事动画(打字命令 + 任务行 ✓ + 方块进度)
 * - 'native' :原生系统开屏复刻(纯色底 + 品牌 ›_ 符号,入场克制的静态样式)
 */
export const ACTIVE_SPLASH_CANDIDATE: SplashCandidate = 'native'
