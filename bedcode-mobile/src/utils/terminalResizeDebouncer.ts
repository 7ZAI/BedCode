/**
 * 终端 resize 分层防抖（对齐桌面端 TerminalPreview / VS Code
 * TerminalResizeDebouncer 语义）
 *
 * 为什么分层：水平 resize 触发整屏 reflow（昂贵），拖拽/旋转时逐帧执行浪费严重，
 * 值得用 100ms 防抖合并；垂直 resize 是行数变化，用户对即时性敏感，必须立即生效。
 * flush() 提供「最终尺寸必达」保证（防抖窗口内的最后一次尺寸可被立即应用，
 * 不必等计时器到期）。
 *
 * 两个对齐 VS Code 的补充分支：
 ...
 * - 0/非法尺寸保护：容器隐藏/旋转过渡中 ResizeObserver 报 0 尺寸，直接忽略
 *   （对齐 VS Code layout() 的 width<=0/height<=0 直接 return），避免把 PTY
 *   缩成 1×1 打乱 shell、恢复时再拉回的无谓 reflow。
 *
