...
pub(crate) fn save_and_push(
    ...
    // 移动端落点固定 MediaStore.Downloads（无 set-download-dir 原语，双端 12/13 差异）
    Ok(())
}
...
mod tests {
    ...
    impl HostStorage for MockHost {
        fn storage_get(&self, key: &str) -> Result<Option<serde_json::Value>, bedcode_plugin_api_mobile::host::HostError> {
            Ok(self.kv.borrow().get(key).cloned())
        }
        fn storage_set(&self, key: &str, value: &serde_json::Value) -> Result<(), bedcode_plugin_api_mobile::host::HostError> {
            self.kv.borrow_mut().insert(key.to_string(), value.clone());
            Ok(())
        }
        fn storage_delete(&self, key: &str) -> Result<(), bedcode_plugin_api_mobile::host::HostError> {
    ...
    impl HostPeer for MockHost {
        fn peer_dial(&self, _endpoint: &serde_json::Value) -> Result<String, bedcode_plugin_api_mobile::host::HostError> { unimplemented!() }
        fn peer_close(&self, _handle: &str) -> Result<bool, bedcode_plugin_api_mobile::host::HostError> { unimplemented!() }
        fn peer_respond_consent(&self, _request_id: &str, _accepted: bool) -> Result<bool, bedcode_plugin_api_mobile::host::HostError> { unimplemented!() }
        fn peer_list_trusted(&self) -> Result<serde_json::Value, bedcode_plugin_api_mobile::host::HostError> { unimplemented!() }
        fn peer_revoke_trusted(&self, _node_id: &str) -> Result<bool, bedcode_plugin_api_mobile::host::HostError> { unimplemented!() }
        fn peer_send_files(&self, _session: &str, _paths: &[serde_json::Value]) -> Result<String, bedcode_plugin_api_mobile::host::HostError> { unimplemented!() }
        fn peer_respond_transfer(&self, _batch_id: &str, _accept: bool) -> Result<(), bedcode_plugin_api_mobile::host::HostError> { unimplemented!() }
        fn peer_set_receive_policy(&self, mode: &str, timeout_secs: u64) -> Result<(), bedcode_plugin_api_mobile::host::HostError> {
