...
//! 损坏的 JSONL 行跳过并记 warn 日志（移动 SDK 无 tracing，走 HostLog）。

use bedcode_plugin_api_mobile::host::{HostFs, HostLog};
...
pub fn init<H: HostFs + HostLog>(host: &H, data_dir: &str) -> anyhow::Result<()> {
    ...
    host.log_debug(&format!("store: data dir initialized: {}", data_dir));
    Ok(())
}

/// 列出全部对话（index.jsonl），按 updatedAt 降序；索引文件缺失视为空列表
pub fn list_conversations<H: HostFs + HostLog>(
    host: &H,
    data_dir: &str,
...
pub fn get_messages<H: HostFs + HostLog>(
    ...
    for line in content.lines() {
        ...
        let parsed = match serde_json::from_str::<ConversationLine>(trimmed) {
            ...
            Err(e) => {
                host.log_warn(&format!(
                    "get_messages: skipping corrupted line ({}): {}",
                    conversation_id, e
                ));
                continue;
            }
...
mod tests {
    ...
    /// 内存版 HostFs + HostLog mock：读写删落到 map（接缝 2 先例）
    struct MockHost {
        files: Arc<Mutex<HashMap<String, String>>>,
    }

    impl HostFs for MockHost {
        fn fs_read(&self, path: &str) -> Result<Option<String>, bedcode_plugin_api_mobile::host::HostError> {
            Ok(self.files.lock().unwrap().get(path).cloned())
        }
        fn fs_write(&self, path: &str, data: &str) -> Result<(), bedcode_plugin_api_mobile::host::HostError> {
            self.files.lock().unwrap().insert(path.to_string(), data.to_string());
            Ok(())
        }
        fn fs_copy(&self, _src: &str, _dst: &str) -> Result<(), bedcode_plugin_api_mobile::host::HostError> {
            Ok(())
        }
        fn fs_delete(&self, path: &str) -> Result<(), bedcode_plugin_api_mobile::host::HostError> {
            self.files.lock().unwrap().remove(path);
            Ok(())
        }
        fn fs_exists(&self, path: &str) -> Result<bool, bedcode_plugin_api_mobile::host::HostError> {
