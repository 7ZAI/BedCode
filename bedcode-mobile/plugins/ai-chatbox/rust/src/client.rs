...
/// 校验流事件通道与命令 streamId 一致（精确匹配，防后缀误放行）
///
/// 独立纯函数：移动 SDK 旧 ABI 的 `extern "C"` 宿主导入在原生测试链接时
/// 不可解析，测试只能覆盖不含 WasmHost 调用的路径（与桌面 wit-bindgen
/// 自动 stub 的机制不同，见 wasm_host.rs 注释）
...
pub fn chat_stream(
    ...
    validate_stream_event(stream_id, request)?;
...
mod tests {
    ...
    #[test]
    fn validate_stream_event_rejects_mismatched_stream_event() {
        ...
        let err = validate_stream_event("s1", &request).unwrap_err();
        assert!(
    ...
    #[test]
    fn validate_stream_event_rejects_suffix_collision() {
    ...
    #[test]
    fn validate_stream_event_accepts_exact_match() {
        let request = json!({
            "method": "POST",
            "url": "https://a.com/chat/completions",
            "streamEvent": "ai-chatbox:stream:s1",
        });
        assert!(validate_stream_event("s1", &request).is_ok());
    }
}
