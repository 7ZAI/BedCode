<template>
  <div class="flex flex-col h-full bg-[var(--mobile-bg-card)]">
    <!-- 头部 -->
    <div class="flex items-center justify-between px-4 py-3 border-b border-[var(--mobile-border)]">
      <span class="text-[var(--font-size-sm)] font-medium text-[var(--mobile-text-secondary)]">
    ...
    <div class="flex-1 overflow-y-auto py-1 flex flex-col">
      <div
        ...
        :key="conv.id"
        class="flex items-center gap-1 px-4 py-2 rounded-xl mx-2 cursor-pointer transition-colors"
      ...
      >
        ...
        <span
          v-else
          class="flex-1 min-w-0 truncate text-[var(--font-size-base)] text-[var(--mobile-text-primary)]"
...
/**
 * ConversationList — 对话列表面板（移动端底部抽屉内使用）
 *
 * 列表 + 新建按钮；项支持重命名、删除；标题为新对话占位文案时显示默认文案。
 */
