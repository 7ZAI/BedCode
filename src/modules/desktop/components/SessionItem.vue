  ...
  <div class="bg-white dark:bg-dark-800 rounded-lg border border-gray-200 dark:border-dark-700 overflow-hidden">
    ...
    >
      ...
      <div class="flex items-center gap-2 flex-shrink-0" @click.stop>
        <Button variant="ghost" size="sm" title="展开详情" @click="toggleExpand">
          <svg
        ...
        <Button variant="ghost" size="sm" title="查看终端" @click="handleView">
        ...
        <!-- 运行中显示停止按钮，已停止显示重启按钮 -->
        <Button v-if="isRunning" variant="ghost" size="sm" title="停止会话" @click="$emit('stop')">
          <svg class="w-4 h-4 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 10a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1v-4z" />
          </svg>
        </Button>
        <Button v-else variant="ghost" size="sm" title="重启会话" @click="$emit('restart')">
          <svg class="w-4 h-4 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
          </svg>
        </Button>
        <Button variant="ghost" size="sm" title="删除会话" @click="$emit('delete')">
          <svg class="w-4 h-4 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
