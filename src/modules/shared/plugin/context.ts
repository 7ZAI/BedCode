...
export function createPluginContext(info: PluginInfo): PluginContext {
  ...
  const session: SessionAPI = {
    async list(): Promise<any[]> {
      ...
      const { listSessions } = await import('@/modules/desktop/composables/useDesktopCommands')
      return listSessions()
    },
    async get(sessionId: string): Promise<any> {
      requirePermission('session.get')
      const { getSession } = await import('@/modules/desktop/composables/useDesktopCommands')
