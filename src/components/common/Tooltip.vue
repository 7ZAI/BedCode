    ...
    <Teleport to="body">
      <Transition name="tooltip">
        <div
          v-if="visible"
          ref="tooltipRef"
          class="fixed z-50 px-3 py-1.5 text-sm bg-dark-700 text-dark-100 rounded-lg shadow-lg border border-dark-600 whitespace-nowrap"
          :style="positionStyle"
        >
          {{ content }}
          <!-- Arrow -->
          <div
            class="absolute w-2 h-2 bg-dark-700 border-dark-600 rotate-45"
            :class="arrowClass"
          ></div>
        </div>
      </Transition>
