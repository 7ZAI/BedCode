<template>
  <div :class="['bg-dark-800 rounded-lg border border-dark-700 overflow-hidden', { 'border-primary-500': hasRunningSessions }]">
    <!-- Config Header (always visible) -->
    <div
      :class="[
        'flex items-center gap-4 px-4 py-3 cursor-pointer transition-colors',
        hasRunningSessions ? 'bg-dark-750' : 'hover:bg-dark-750'
      ]"
      @click="$emit('edit')"
    >
      ...
      <div class="flex-1 min-w-0">
        <h3 class="font-medium text-white truncate">{{ config.name }}</h3>
        <p class="text-dark-400 text-sm truncate">{{ config.workingDir }}</p>
      </div>

      <!-- Command (always visible on desktop) -->
      <div class="flex items-center gap-2 text-dark-400 text-sm flex-shrink-0">
      ...
      <div class="flex items-center gap-2 flex-shrink-0" @click.stop>
        ...
        <div
          ...
          class="flex items-center gap-1 mr-2 text-green-400 text-sm"
        >
    ...
    <div v-if="runningSessions.length > 0" class="border-t border-dark-700">
      <div
        class="flex items-center gap-2 px-4 py-2 bg-dark-850 cursor-pointer hover:bg-dark-800 text-dark-400 text-sm"
      ...
      <div v-if="isExpanded" class="bg-dark-900">
        <div
          v-for="session in runningSessions"
          :key="session.id"
          class="flex items-center justify-between px-4 py-3 border-b border-dark-700 last:border-b-0 hover:bg-dark-800 cursor-pointer"
        ...
        >
          ...
          <div class="flex items-center gap-2">
            <span class="text-dark-400 text-sm">{{ getSessionTime(session) }}</span>
