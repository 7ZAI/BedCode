...
`@bedcode/plugin-sdk-desktop` 与 `@bedcode/plugin-sdk-mobile` 是**双栈 SDK 包**，各包含前端 TS 部分和 Rust crate 部分，发布到两个 registry：

| SDK | npm 包 | crates.io crate |
|-----|--------|-----------------|
| 桌面端 | `@bedcode/plugin-sdk-desktop` | `bedcode-plugin-api` |
| 移动端 | `@bedcode/plugin-sdk-mobile` | `bedcode-plugin-api-mobile` |
...
npx @bedcode/plugin-sdk-mobile create com.example.demo "Demo" --registry
```

生成的依赖声明：
```jsonc
"@bedcode/plugin-sdk-mobile": "^0.1.0"
```
```toml
...
- **workspace 成员发布**：desktop SDK 是 `bedcode-desktop` npm workspace 成员（`packages/*`），发布用 `npm publish -w @bedcode/plugin-sdk-desktop`；`prepublishOnly` 会自动执行 build + test
