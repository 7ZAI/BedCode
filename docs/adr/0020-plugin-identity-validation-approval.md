...
- 前端插件通道：`plugin_*` 命令的身份由宿主签发的凭证绑定，参数里的 `plugin_id` 只作**目标**（插件令牌只能操作自己，宿主凭证可操作任意目标）；「前端与被审插件同权共生」这一事实不变——凭证提高的是冒充门槛，不是隔离边界
...
- **2026-09-22 — 前端通道身份（审计票 06）**：本 ADR 的身份模型此前只覆盖 guest/WASM 通道；
  Tauri 命令面**拿不到调用者身份**（同一 webview 内宿主与全部插件前端同权，
  `#[tauri::command]` 只给声明参数），于是 `plugin_*` 桥的门禁查的是**参数里自报的 `plugin_id`**：

  1. `plugin_storage_get/set/delete`：任一插件前端可读写他人存储；
  2. `plugin_invoke`：**连权限门都没有**（只查目标是否激活）→ 插件 A 可驱动插件 B 的命令；
  3. `plugin_fs_auth_respond`：授权请求事件是广播的 → 插件可替用户「同意」自己的文件访问请求。

  本次把「身份」从参数搬到**宿主签发的凭证**（两级，因为「谁先请求」在 webview 内不可验证）：

  - **loader 会话密钥**（宿主前端面）：每次页面加载**首个调用者生效**，宿主前端在导入任何插件模块
    之前取得（`pluginLoader.loadAll()` 首行），由 Tauri `on_page_load` 钩子在页面加载时重置；
  - **插件通道令牌**（插件面）：`plugin_channel_token(plugin_id, loader_session)` 用 loader 密钥换取，
    插件运行态才签发、**停用即回收**；
  - 每个插件面命令都带 `credential`：令牌只能操作自己（不符即拒，错误点名两侧），loader 密钥可操作
    任意目标（宿主职权）；凭证无效一律拒绝，**不落回「按参数放行」**。

  **同批退役 `sandbox` 字段**（裁决 2）：它承诺 `isolated` 而实现只有「非 inline 直接跳过」，
  且前端与宿主同权共生、不存在隔离。字段从 manifest（Rust SDK + 两份 TS 副本）、四个随包
  `plugin.json`、loader 分支与前端类型中删除；旧产物带该键照常加载（未知字段约定）且不再回写。
  SDK 公开字段退役，**下次发布 SDK 时需按 0.x 契约记 breaking**（本 ADR 只登记事实，不改版本号）。

  **边界（不宣称硬隔离）**：本批关闭的是「一行 `invoke` 自报 plugin_id」这条通道，不是
  「插件前端与宿主同权」这个事实（同 realm 的原型/时序手法理论上仍可尝试）——这正是删除
  `isolated` 承诺而不是半实现它的原因。宿主管理面命令（activate / install / uninstall / approve /
  dev-reload）按裁决保持不绑定；插件前端的 Tauri 事件订阅面无法按调用者绑定身份，两者均在票面登记。
