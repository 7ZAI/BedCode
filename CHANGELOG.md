...
#### Peer transfer orchestration sinks to the plugin — host `peer_engine_*` collapse into handle tables + engine event bridge (desktop, **v30 + v31**)
- Transfer task orchestration was still living host-side (active task state machine, send concurrency gate 1..=8 default 3, history caps 200/100, serve-side double accounting, `peer_name` resolution, cancel reason-code mapping, pull row pre-registration + semaphore) — same drift shape as the session-engine batch. It all moves into the `com.bedcode.file-transfer` plugin as an **event-reduction state machine** (`transfer_store.rs`: row creation / progress / terminal with direction-aware reason codes / cap / retry replay — single source of truth in the plugin private DB)
- **Ticket 1 (v30, additive)**: `host-peer.active-transfers` (active batch projection for first-screen bootstrap) + `collect-outgoing` (directory-recursive source enumeration with in-batch name dedupe); raw engine events bridge onto the bus as `peer:transfer-event` / `peer:receive-event` (payloads carry `tsMs` — wasm32 has no clock), dual-written with the legacy snapshot topics during transition
- **Ticket 2 (plugin side)**: event reducer consumes the raw stream (snapshot merge becomes correction + reconciliation — drift is logged via `reconcile_diff`); plugin-side **send concurrency gate** (`PENDING_SENDS` queue, dispatches when a running slot frees); `peer_name` resolved from the plugin's own device snapshot (short-fingerprint fallback); `peer_active_transfers` bootstrap on activation
- **Ticket 3 (v31, breaking)**: `host-peer.resume-all-transfers` **deleted** ("resume all" orchestration belongs to the plugin — iterate its paused batches; `file-transfer.resume-all` command keeps working via per-batch resume); `send-files` narrowed to "one call = one session started immediately" — the host concurrency gate is gone and the legacy `concurrency` payload field now **fails visibly at call time** (rebuild hint); the legacy snapshot topics `peer:transfer` / `peer:receive` are retired from the bus mapping; `pull-files` drops the semaphore and row pre-registration (per-file sessions start immediately, a `pull-started` engine fact event feeds the plugin's reducer); `active-transfers` implementation switches to the three handle-face projections (same wire shape); host residue deleted — `peer_engine_transfer.rs` rewritten around `SendSessionHandle` (CancelToken / PauseSlot / epoch / sources, sources kept for redial resume), source collection split into `source_collect.rs`, `peer_engine_receive.rs` keeps only the offer-receipt table + event bridge + policy gate (concurrency setting removed), and a 15-symbol anti-reintroduction lock (`retired_peer_transfer_orchestration_is_not_reintroduced`) is mutation-tested (inject → red → restore → green)
- Old pre-v31 artifacts fail **at instantiation** with a rebuild hint naming `resume-all-transfers`; pre-v31 artifacts that still send the `concurrency` field fail at the `send-files` call with the same pointer (fail-visible, both layers)
- Mobile is untouched and explicitly out of scope (`host-peer` is desktop-only; the wire data-plane protocol is unchanged) — recorded impact list: `.scratch/2026-09-25-peer-transfer-orchestration-downsink/mobile-impact.md`

#### Desktop wasm-plugin semantics become "wasm apps" — source dir `plugins/` → `wasm-apps/`, menu "应用中心" (desktop, surface-only)
- 2026-09-25 user ruling: after the host went business-free, a plugin mounted on the desktop host behaves like a self-contained application, so the desktop surface terminology changes to **wasm app** while every internal implementation stays `plugin`. Mobile is untouched (still `plugins/` + `@bedcode/plugin-*`)
- Source directory moved `bedcode-desktop/plugins/` → `bedcode-desktop/wasm-apps/` (`git mv`, four apps: ai-chatbox / file-transfer / agent-hub / terminal-session); path references updated across the build chain (`scripts/plugin-build.js` / `plugin-dev.js` / `dev-run.js` / `plugin-watch.js`, `package.json` `plugins:build:*`, `pnpm-workspace.yaml`, `scripts/package-plugins.mjs` `DESKTOP_PLUGINS`, `scripts/wasip3-toolchain.sh`), `tailwind.config.js` content, `vitest.config.ts` include, the tailwind content coverage lock test, the `terminal-session` plugin contract test, and all active docs (AGENTS.md, desktop code-map, plugin-dev guides, auth doc, diagrams README, workspace files)
- npm package names `@bedcode/plugin-*` → `@bedcode/wasm-app-*` (lockfile refreshed via `pnpm install`); the SDK scaffold CLI (`plugin-sdk-desktop/bin/cli.js`) now emits `@bedcode/wasm-app-*` for new apps (`bedcode_plugin_*` crate identities unchanged)
- Frontend visible copy: sidebar menu becomes **应用中心** (App Center), all `desktop.plugin.*` i18n values switch 插件→应用 (zh-CN + en; key names unchanged), approval dialog & load/uninstall flows follow
- **Unchanged (internal)**: Rust kernel (`wasm_core`, `PluginHost`, `plugin_id`, `resources/plugins/` artifacts, runtime `app_data_dir/plugins`), frontend `src/plugin/`, routes `/plugins`, component names, plugin IDs `com.bedcode.*`, SDK package name `@binblink/bedcode-plugin-sdk-desktop`, and everything mobile
...
- **Websocket business downsink final gates — WS-output performance baseline + evidence + doc sync (desktop, websocket business downsink ticket 09)**: the hard-cut batch closes out with its performance gate (spec §9.3) and full verification. A new WS-output-path probe (`ws_output_perf.rs`, alongside the existing PTY-output baseline `terminal_output_perf.rs`) measures the plugin `ring-fetch → WIT binary → WS send` leg end-to-end over a real Actix server + real terminal-session plugin + real bash PTY + real WS client: normal case (ring-safe 192 KiB, no eviction) delivers every byte with zero truncation; stress case (10 MiB throttled ≈10 MB/s continuous production) sustains multi-MB/s drain with ring eviction re-anchoring recorded as `ring_resync` frames (never silent loss). `pty_session_chain` now also asserts the retired `/ws/event` and `/ws/terminal/session/{id}` routes 404 without upgrade, and the old-artifact rebuild hint gains the v28 deletion (`host-events.broadcast-sync`) with a named rebuild guidance. The session-center frontend test mock was completed for the ticket-05 event subscription (missing `events` on the plugin context caused 10 unhandled rejections under vitest). ADR 0022 gains the v19 revision record (v28 batch) and the dual-platform deviation table moves to **desktop v28 / mobile 11**
...
- **Desktop splash screen gained a light theme (desktop)**: `SplashLoading.vue` used to paint with a fixed set of dark brand constants ("不随主题切换" was the documented intent), so light-mode users booted into a dark screen. The colours are now two palette blocks — a light base (paper ground + ink text, endpoints read the current `--bg-*` / `--text-*` tokens, so the warm/cool/forest/… palette carries the splash along) and an `html.dark` override that keeps the existing brand gradient verbatim. Every rule references a `--splash-*` variable, and a new lock (`src/__tests__/components/SplashLoadingTheme.test.ts`) fails when one block is missing, when the two token sets drift apart, or when a literal colour reappears outside them. Light text alphas sit above the dark ones (ink 78% / 65% vs 55% / 35%) because low-alpha ink on paper falls under AA at 10–13px, and the neon glows are replaced by border + shadow depth. The pre-Vue static screen in `index.html` got a `prefers-color-scheme: light` branch — it runs before the tokens and before the SQLite theme setting load, so only the system preference is readable there (the same limitation the mobile static screen documents), and its hand-mirrored hex values are locked against `style.css`. Out of scope: the native window `backgroundColor` (`#15130F` in `tauri.conf.json`) stays a single dark value
- **HTTP route registration sinks into plugins — `host-http` server domain + dynamic route registry (desktop, ABI v29)**: the host HTTP transport's route-registration power moves to plugins (user rulings 1–7): plugins register their own routes at activation via new `host-http.register-endpoint` / `unregister-endpoint` (internal endpoint segment + optional external URL alias with `{id}` template segments + methods + `jwt`/`none` auth tier; `network:http` permission, same as the frontend `http.registerEndpoint`). The host keeps only the generic registry (`server/http/registry`, keyed by owner+host-path+method, cross-plugin host-alias conflict is fail-visible with the incumbent kept, per-owner purge on deactivation, host-owned surfaces `/api/health` and `/api/plugin/*` are reserved), the generic gateway (static `BUSINESS_ROUTES` table / `BusinessDomain` / `SESSION_PLUGIN` / `FallbackPolicy::HostImplementation` deleted; template captures are injected to the plugin as `params` and never used to construct paths), the verification engine, and the auth-center policy framework (public-path determination now reads the registry tier instead of the hard-coded `/api/auth/` prefix; the auth-center role is discovered by scanning activated plugins that export the `auth-policy` capability instead of a hard-coded plugin id; the WS plugin-endpoint `verify_endpoint_jwt` now also enforces the connection policy, aligning with the HTTP middleware). Plugin-name uniqueness becomes an explicit activation gate (duplicate manifest.id → activation fails, namespace isolation). `contributes.httpEndpoints` and the never-consumed `toolProviders` static declaration surfaces are retired (manager registry's http-endpoint table deleted). `com.bedcode.terminal-session` migrates its 42 endpoints to code registration (business 10 + auth 7 + task 17 + sessions REST 7 + terminal-bg 1): `/api/sessions*` REST (list/start/stop/resize/input/history/remove) moves into the plugin's new `sessions_http` domain (host `session_controller.rs` deleted, response shapes byte-locked), and `/static/terminal-bg` stays host-served but registry-gated (binary images cannot pass the text-only `host-fs.read`; URL ownership/lifecycle belongs to the plugin). External URLs are unchanged; a deactivated plugin's aliases now 404 instead of 200+1007 (fail-visible). Old (v28) artifacts still instantiate (interface additions only) but cannot register routes; a new-artifact-on-old-host instantiation failure gains an upgrade hint. New fixture `packages/plugin-http-test` + `http_e2e` closed loop (registered routes / template capture / `_http_endpoint` echo / owner unregister idempotency). Mobile explicitly out of scope (ADR 0022 deviation)
...
#### Desktop
- **Plugin re-activation while its view is open leaves the subtree on a revoked channel credential** — after a plugin is deactivated and re-enabled (or dev-watcher-reloaded) with e.g. the Session Center view open, the view's `onMounted` `load()` ran with the *pre-deactivation* `PluginContext`: `PluginViewHost` / `PluginSettingsSection` only `provide('pluginContext', …)` in their `setup`, and the old code's props-change `watch` re-provide is dead code — Vue's `provide()` only works when called synchronously during `setup` (verified against Vue 3.5 in a test probe). The stale context's channel token is revoked on deactivate, so every plugin-face command (`session.config.list` / `session.list` / storage…) was rejected with `缺少有效通道凭证` → user-visible `会话数据加载失败` + `发生了未知错误` toasts. Fix: (1) registry gains a reactive context projection + `contextIdentity(pluginId)` (stable per-object id, `contextsIndex` shallow ref, same pattern as `viewsIndex`); (2) both hosts now render the inner view through a keyed `PluginContextProvider` (new component) that re-mounts — and thus re-provides in `setup` — whenever the plugin's context object identity changes; (3) the loader registers the fresh context in the registry **before** `module.activate()`, so views (re-)mounting mid-activation always resolve the current context, and clears it again if activation fails. Regression tests: `PluginViewHost.test.ts` (re-activation re-provides the new context, same-object re-set does not churn) + `contextIdentity` contract tests in `registry.test.ts`; E2E-verified against the running dev app (plugin dev-watcher reload with the Session Center view open: zero `缺少有效通道凭证`, list commands re-ran on the freshly issued token)
