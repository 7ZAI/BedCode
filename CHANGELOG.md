...
#### Desktop
- Recorded while widening the permission locks: **five** integration test targets do not compile at HEAD — `ws_session_route`, `pty_session_chain`, `ws_auth_rules`, `http_auth_biometric`, `broadcast_shutdown`. Every one of them imports symbols retired earlier (`server::services::pairing_service`, `AppContextBuilder::pairing_service`, `utils::auth::QrTokenManager`, `SessionManager::from_database`, `restart_session`), and none of the errors touches code changed by the audit tickets. `cargo test --lib` (1091 green) hides all of them, as does any `--lib`-only gate. Cleaning them up is a separate item, deliberately not folded into an audit ticket
