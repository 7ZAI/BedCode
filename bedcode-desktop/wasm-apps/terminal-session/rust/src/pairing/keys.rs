...
/// 会话中心密钥获取策略：get-or-create（首启随机生成 + 持久化）
///
...
pub fn get_or_create_jwt_key(store: &impl SecretStore) -> Result<Vec<u8>, String> {
    if let Some(existing) = store.get(JWT_SECRET_KEY_ID)? {
        let key = hex::decode(existing).map_err(|e| format!("stored jwt key not hex: {}", e))?;
...
mod tests {
    ...
    fn second_call_reads_back_persisted_key() {
        ...
        store
            .set(
                JWT_SECRET_KEY_ID,
                &hex::encode(vec![0xabu8; JWT_SECRET_KEY_LEN]),
            )
            .unwrap();
    ...
    fn corrupt_stored_key_fails_loudly() {
        ...
        store2
            .set(JWT_SECRET_KEY_ID, &hex::encode([0u8; 16]))
            .unwrap();
