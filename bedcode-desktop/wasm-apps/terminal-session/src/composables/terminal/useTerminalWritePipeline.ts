/**
 * 终端写入管线（插件侧，自宿主 TerminalPreview 拆分产物迁入）
 *
 * 职责：
 ...
 *
 * 与宿主版本差异（票 01a 迁入适配）：
 * - 数据源解耦：宿主版内部构造 `useTerminalOutputStreamChannel`（Tauri Channel
...
/** 输出源交付的一帧（已通过游标连续性校验/去重，直接入队） */
export interface TerminalOutputFrame {
  data: Uint8Array
}

/** 输出源回调（调用方实现；对应宿主版 terminalStreamOptions 三回调） */
export interface TerminalOutputSink {
  onData: (frame: TerminalOutputFrame) => void
  onReset: () => void
...
/**
 * 创建写入管线。返回：
 * - `attachSource(source)`：调用方把输出源接入（订阅时调用；源停止时自行断开）
 * - 生命周期：`dispose()` 卸载时清理挂起调度
 */
export function useTerminalWritePipeline(
  ctx: TerminalKernelContext,
  options: TerminalWritePipelineOptions,
) {
  ...
  function attachSource(): TerminalOutputSink {
    return {
      onData: ({ data }: TerminalOutputFrame) => {
        enqueueOutput(data)
