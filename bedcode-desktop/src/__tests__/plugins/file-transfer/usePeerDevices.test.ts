/**
 * usePeerDevices 编排测试（ticket 01）
 *
 * mock 最小 PluginContext（commands.execute 记录调用 + events.on 捕获处理器
 * 手动派发），只测编排逻辑不测渲染。场景矩阵对齐被迁移的宿主 usePeerDevices
 * 测试：start 幂等、发现快照事件整表替换、活跃兜底切换、连接/断开命令路由、
 * denied/unreachable 如实上报、WS 控制面与对等连接态语义隔离。
 */
