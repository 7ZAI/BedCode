 ...
 * Rust DTO 字段名集合（文件头注明 Rust 源文件），工厂每次产出时用
 * assertDtoFields 做运行时键集合断言——Rust 侧新增字段后清单被同步时，
 * 任何未更新的 fixture 都会让引用它的测试立刻失败（显式告警，而非静默漂移）。
 *
 * 历史上出现过 active_task 字段缺失、approvalTimeoutSec 模板访问等前后端
 * 契约 bug，此机制是契约防线的第一层（第二层为各 fixture 文件内的
 * 类型级 Equals 断言，供编辑器与未来 CI 类型检查捕获）。
 */
 ...
 *
 * 仅用于 fixture 类型与前端消费类型能严格对齐的 DTO（如 ServerStatusInfo）；
 * 前端类型存在遗留别名（camelCase 双写）或线协议字段被前端忽略时改用
 * 运行时断言，不在此做严格相等。
 */
