 ...
 * 归零主动回执拒绝（宿主引擎 TTL 为权威兜底）。与 usePeerTransfers /
 * usePeerDevices 同款模块级单例模式。
 */
import { computed, ref } from 'vue'
import { invoke } from '@tauri-apps/api/core'
import { listen, type UnlistenFn } from '@tauri-apps/api/event'
import type { PeerTransferTask, PeerTransferFile } from './usePeerTransfers'
...
/** 接收设置（后端 get_peer_receive_settings；桌面端含 downloadDir） */
...
/** 更新接收落点目录（仅桌面端命令存在；None 恢复缺省），成功后回读 */
async function setDownloadDir(path: string | null): Promise<boolean> {
  try {
    await invoke('set_peer_download_dir', { path })
    await loadSettings()
    return true
  } catch (error) {
    console.error('[PeerReceiving] set download dir failed:', error)
    return false
  }
}

/** 系统文件夹选择器选落点（用户取消返回 null） */
async function pickDownloadDir(): Promise<string | null> {
  try {
    const picked = await invoke<string | null>('peer_pick_download_dir')
    return Array.isArray(picked) ? (picked[0] ?? null) : picked
  } catch (error) {
    console.error('[PeerReceiving] pick download dir failed:', error)
    return null
  }
}
 ...
 * 两件事：倒计时展示值刷新 + 归零自动回执拒绝（宿主 TTL 兜底，双保险防
 * 前端挂起漏拒）。心跳为模块单例，随应用生命周期走。
 */
