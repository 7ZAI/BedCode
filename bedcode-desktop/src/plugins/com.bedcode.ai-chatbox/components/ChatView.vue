  ...
  <div class="h-full flex flex-col bg-white dark:bg-dark-900">
    <header class="px-4 py-2 flex items-center justify-between border-b border-slate-200 dark:border-dark-700 bg-slate-50 dark:bg-dark-800">
      <div class="flex items-center gap-2">
        <select
          v-if="config.hasProvider"
          :value="config.activeProviderName"
      ...
      <div class="flex items-center gap-1">
        <button
          ...
          title="模型配置"
          @click="config.showProviderManager = !config.showProviderManager"
        >
        ...
        <button
          ...
          title="新对话"
          @click="chat.newConversation(config.activeProviderName)"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <ProviderManager
      v-if="config.showProviderManager"
      :providers="config.providers"
      :active-provider-name="config.activeProviderName"
      @set-active="config.setActiveProvider"
      @remove="config.removeProvider"
      @add="config.addProvider"
    />

    <div v-if="!config.hasProvider" class="flex-1 flex flex-col items-center justify-center p-6 text-center">
      ...
      <button
        ...
        @click="config.showProviderManager = true"
      >
        配置模型
      </button>
    </div>
    ...
    <template v-else>
      ...
      <div class="border-t border-slate-200 dark:border-dark-700 p-3">
        <ChatInput
          :disabled="chat.sending || !config.activeProvider"
          placeholder="输入消息..."
    ...
    <PromptOptimizeDialog
      :show="optimizerState.showDialog"
      :optimizing="optimizerState.optimizing"
      :original="optimizerState.originalText"
      :optimized="optimizerState.optimizedText"
      :error="optimizerState.errorMessage"
