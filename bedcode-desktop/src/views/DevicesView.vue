          ...
          >
            ...
            <div class="flex items-center justify-between gap-3 mb-4">
              <div class="min-w-0">
                ...
                >
                  {{ t('desktop.device.pairingCodeTitle') }}
                </h4>
              </div>
            ...
            <div class="flex flex-col items-center text-center">
              ...
              <template v-else>
                ...
                >
                  ...
                  >
                    {{ t('desktop.device.pairingCodePlaceholder') }}
                  </p>
                </div>
                <p
                  class="text-[calc(12px*var(--ui-scale))] text-[var(--text-secondary)] leading-relaxed"
          ...
          >
            ...
            <div class="flex flex-col items-center text-center">
              <template v-if="qr.hasQr.value">
                <!-- 白底衬底保证二维码在暗色模式下可读 -->
                <div
                  class="inline-block bg-white p-3 rounded-lg border border-[var(--border)] mb-4"
                >
        ...
        <div v-else class="space-y-6">
          ...
          <section>
            <h3 class="wb-section-title">
              {{ t('desktop.device.sectionOnline') }}
              ...
              >
                ...
                >
                  ...
                  <template v-if="device.lastSeen">
                    <span class="text-[var(--border)]">·</span>
                    <span>{{
                      t('desktop.device.lastSeen', { date: formatDate(device.lastSeen) })
                    }}</span>
                  </template>
                  <span class="text-[var(--border)]">·</span>
                  <span>{{
          ...
          <section>
            <h3 class="wb-section-title">
              {{ t('desktop.device.sectionOffline') }}
...
const ipOptions = computed(() => ipv4Addresses.value.map((ip) => ({ value: ip, label: ip })))
...
onMounted(async () => {
  ...
  // 监听设备连接事件
  deviceConnectedListener = await listen<DeviceConnectionInfo>(
    'device-connected',
    async (event) => {
  ...
  // 监听设备断开事件
  deviceDisconnectedListener = await listen<DeviceConnectionInfo>(
    'device-disconnected',
    (event) => {
      const fp = (event.payload as any).fingerprint
      if (fp) {
