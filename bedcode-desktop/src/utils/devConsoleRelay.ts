/**
 * 前端 console 日志转发（仅 dev 生效）——AI agent 排查前端问题的落盘通道
 *
 * dev 构建下覆盖 console.log/debug/info/warn/error，把输出批量经 IPC
 * 转发到 Rust 命令 `report_frontend_log`（仅 dev 注册），写进
 * runtime.*.log（target=`frontend`）。release 构建 Rust 侧不注册该命令，
...
export function installDevConsoleRelay(envDev: boolean = import.meta.env.DEV): () => void {
  ...
  const flush = () => {
    ...
    invoke('report_frontend_log', { logs: batch }).catch(() => {
      if (!reportedFailure) {
        ...
        original.warn.call(console, '[devConsoleRelay] 日志转发失败（仅 dev 生效，release 无此命令）')
      }
    })
  }
  ...
  // console.log 语义最接近 info，但对应后端 debug 级（dev 落盘 file_level 强制 debug，trace 不落盘）
