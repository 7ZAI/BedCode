export default {
  desktop: {
    ...
    plugin: {
      viewNotFound: '应用视图未找到',
      windowLoading: '正在加载应用窗口…',
      windowTargetMissing: '应用窗口视图不可用：{pluginId} / {viewId}',
      searchCommands: '搜索应用命令...',
      noCommands: '没有找到匹配的命令',
      // 桌面端语义（2026-09-25）：wasm 插件对外称 wasm 应用；运行时安装目录
      // ~/.bedcode/plugins/ 是内部实现路径，保持不变
      title: '应用中心',
      noPlugins: '暂无已安装应用',
      noPluginsHint: '将应用放入 ~/.bedcode/plugins/ 目录',
      config: '配置',
      enabled: '启用',
      enabledSection: '已启用',
      disabledSection: '未启用',
      ...
      perm: {
        unknown: '未知权限',
        storage: { title: '存储', desc: '读写应用本地存储' },
        'terminal:input': {
          title: '终端输入',
          desc: '向终端会话发送输入',
        ...
        'ui:dialog': { title: '应用对话框', desc: '弹出宿主风格对话框' },
        ...
        'app:cli': { title: '随包 CLI', desc: '把应用自带的命令行工具安装到 PATH' },
        ...
        auth: { title: '密钥托管', desc: '读写本应用在宿主托管的凭据' },
        'pty:spawn': {
          title: '私有终端创建',
          desc: '创建与终止应用自有伪终端',
          risk: '可创建伪终端并在你的机器上启动任意 shell',
        },
        'pty:io': { title: '私有终端读写', desc: '读写与调整应用自有终端会话' },
        'crypto:aead': {
          title: '对称加密',
      ...
      // ==================== 权限审批（ADR 0020） ====================
      approve: {
        hint: '该应用的权限清单尚未确认，启用前需人工批准。',
        action: '审批权限',
        title: '批准应用权限',
        desc: '「{name}」请求以下权限。批准后这些权限才会生效；应用文件在批准后被修改时，批准会自动撤销。',
        highRiskLabel: '高危',
        empty: '该应用未请求任何权限',
        confirm: '批准',
        cancel: '取消',
      ...
      // ==================== 加载/卸载应用（zip） ====================
      loadPlugin: '加载应用',
      loadPluginHint: '从本地 zip 应用包安装（dist/plugin-packages 产物）',
      loadPluginSheetTitle: '加载应用',
