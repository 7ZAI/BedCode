export default {
  desktop: {
    ...
    plugin: {
      ...
      // 权限元数据（覆盖 SDK 词汇表全部条目；risk 仅高危位有，供审批弹层红色强调）
      perm: {
        unknown: '未知权限',
        ...
        'ui:statusbar': { title: '状态栏项', desc: '在状态栏与标题栏注册条目' },
        'ui:dialog': { title: '插件对话框', desc: '弹出宿主风格对话框' },
        ...
        'process:run': {
          title: '进程执行',
          desc: '在宿主机器上执行外部命令',
          risk: '可在你的机器上执行任意命令与脚本',
        },
        'app:cli': { title: '随包 CLI', desc: '把插件自带的命令行工具安装到 PATH' },
        peer: { title: '对等网络', desc: '发现设备、建立连接与收发文件' },
        mdns: { title: '局域网发现', desc: '广播与浏览局域网服务' },
        'ws:client': { title: 'WebSocket 出站', desc: '主动连接外部 WebSocket 服务' },
        'ws:server': { title: 'WebSocket 入站', desc: '在局域网内监听 WebSocket 端点' },
        auth: { title: '密钥托管', desc: '读写本插件在宿主托管的凭据' },
        'pty:spawn': {
          title: '私有终端创建',
          desc: '创建与终止插件自有伪终端',
      ...
      // ==================== 权限审批（ADR 0020） ====================
      approve: {
        hint: '该插件的权限清单尚未确认，启用前需人工批准。',
        action: '审批权限',
        title: '批准插件权限',
        desc: '「{name}」请求以下权限。批准后这些权限才会生效；插件文件在批准后被修改时，批准会自动撤销。',
        highRiskLabel: '高危',
        empty: '该插件未请求任何权限',
        confirm: '批准',
        cancel: '取消',
