  ...
  <div class="p-6 space-y-4">
    ...
    >
      <div class="flex items-center gap-3">
        <span
          v-if="isSvgIcon(record.manifest.icon)"
          class="w-8 h-8 flex items-center justify-center text-[var(--color-primary)]"
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-[11px] text-[var(--text-tertiary)] truncate">
            {{ record.id }} · v{{ record.manifest.version || '-' }}
          </p>
        </div>
        <span
          class="px-2 py-0.5 rounded-tag text-[11px] flex-shrink-0"
      ...
      <p
        v-if="record.state === 'error' && record.error"
        class="text-xs text-red-500 bg-red-500/10 rounded-lg px-3 py-2 break-all"
      >
        {{ record.error }}
      </p>
      ...
      <div class="flex items-center gap-2">
        ...
        >
          {{
            record.state === 'deactivated'
              ? t('devshell.plugins.activate')
              : t('devshell.plugins.deactivate')
          }}
        </button>
      </div>
      ...
      <div class="grid gap-2 text-xs md:grid-cols-2">
        <div v-if="sidebarPanels.filter((x) => x.pluginId === record.id).length">
          ...
          <p
            v-for="x in sidebarPanels.filter((y) => y.pluginId === record.id)"
            :key="x.panel.id"
        ...
        <div v-if="toolboxPages.filter((x) => x.pluginId === record.id).length">
          ...
          <p
            v-for="x in toolboxPages.filter((y) => y.pluginId === record.id)"
            :key="x.page.id"
            class="px-2 py-1 rounded-tag bg-[var(--bg-hover)]"
          >
            {{ x.page.title }}
        ...
        <div v-if="fileHandlers.filter((x) => x.pluginId === record.id).length">
          ...
          <p
            v-for="x in fileHandlers.filter((y) => y.pluginId === record.id)"
            :key="x.handler.id"
        ...
        <div v-if="endpoints.filter((x) => x.pluginId === record.id).length">
          ...
          <p
            v-for="x in endpoints.filter((y) => y.pluginId === record.id)"
            :key="x.path"
            class="px-2 py-1 rounded-tag bg-[var(--bg-hover)] font-mono"
          >
            {{ x.path }}
        ...
        <div v-if="titleBarItems.filter((x) => x.pluginId === record.id).length">
          ...
          <p
            v-for="x in titleBarItems.filter((y) => y.pluginId === record.id)"
            :key="x.item.id"
            class="px-2 py-1 rounded-tag bg-[var(--bg-hover)]"
          >
            {{ x.item.label }}
        ...
        <div v-if="statusBarItems.filter((x) => x.pluginId === record.id).length">
          ...
          <p
            v-for="x in statusBarItems.filter((y) => y.pluginId === record.id)"
            :key="x.item.id"
            class="px-2 py-1 rounded-tag bg-[var(--bg-hover)]"
          >
            {{ x.item.label }}
        ...
        <div v-if="pageToolbarItems.filter((x) => x.pluginId === record.id).length">
          ...
          <p
            v-for="x in pageToolbarItems.filter((y) => y.pluginId === record.id)"
            :key="x.item.id"
            class="px-2 py-1 rounded-tag bg-[var(--bg-hover)]"
          >
        ...
        <div v-if="inputExtensions.filter((x) => x.pluginId === record.id).length">
          <p class="text-[var(--text-tertiary)] mb-1">
            {{ t('devshell.plugins.inputExtensions') }}
          </p>
          <p
            v-for="x in inputExtensions.filter((y) => y.pluginId === record.id)"
            :key="x.ext.id"
            class="px-2 py-1 rounded-tag bg-[var(--bg-hover)]"
          >
            {{ x.ext.label }}
        ...
        <div v-if="terminalToolbarItems.filter((x) => x.pluginId === record.id).length">
          ...
          <p
            v-for="x in terminalToolbarItems.filter((y) => y.pluginId === record.id)"
            :key="x.item.id"
            class="px-2 py-1 rounded-tag bg-[var(--bg-hover)]"
          >
            {{ x.item.label }}
