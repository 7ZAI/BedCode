...
<template>
  <div class="p-6">
    ...
    <div
      v-if="toolboxPages.length === 0"
      class="py-16 flex flex-col items-center gap-2 text-center"
    >
    ...
    <div class="grid gap-4" style="grid-template-columns: repeat(auto-fill, minmax(180px, 1fr))">
      ...
      >
        <span
          v-if="isSvgIcon(entry.page.icon)"
        ...
        <span class="text-[11px] text-[var(--text-tertiary)] truncate max-w-full">{{
          entry.pluginId
        }}</span>
      </button>
    </div>
