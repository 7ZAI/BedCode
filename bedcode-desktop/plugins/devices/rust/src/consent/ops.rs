...
pub fn decide(
    ...
    if trusted_node_ids.iter().any(|id| id == &request.node_id) {
        return Ok(ConsentDecision::accept(DecisionReason::Trusted, &request.request_id));
    }

    // 2. 用户显式意向 / 3. 未知 peer 无意向 → ask
    match request.user_decision {
        Some(UserDecision::Accept) => Ok(ConsentDecision::accept(DecisionReason::Explicit, &request.request_id)),
        Some(UserDecision::Deny) => Ok(ConsentDecision::deny(&request.request_id)),
        Some(UserDecision::OneTimeAccept) => {
            Ok(ConsentDecision::accept(DecisionReason::OneTime, &request.request_id))
        }
...
/// 互调 api `auth.decide-consent` 实现（wasm 运行时）：收集可信集 → 决策
///
...
mod tests {
    ...
    fn request(request_id: &str, node_id: &str, user_decision: Option<UserDecision>) -> ConsentRequest {
        ConsentRequest {
    ...
    fn one_time_accept_grants_request_without_trust() {
        ...
        let d = decide(&r, &[]).expect("decide");
        assert_eq!(d.decision, DecisionKind::Accept);
        assert_eq!(d.reason, Some(DecisionReason::OneTime), "one_time 判别消费方不落信任");
    }
    ...
    fn trust_match_is_exact_node_id() {
        ...
        let d = decide(&r, &[NODE_A.to_string()]).expect("decide");
        assert_eq!(d.decision, DecisionKind::Accept, "NODE_B 不在可信集 → 走用户意向");
        assert_eq!(d.reason, Some(DecisionReason::Explicit));
    ...
    fn decision_json_shape_is_contract() {
        ...
        let json = serde_json::to_value(decide(&request("req-7", NODE_B, None), &[]).expect("decide")).unwrap();
        ...
        let json2 =
            serde_json::to_value(decide(&request("req-8", NODE_A, None), &[NODE_A.to_string()]).expect("decide"))
                .unwrap();
        ...
        // one_time：wire 判别
        let json3 =
            serde_json::to_value(decide(&request("req-9", NODE_B, Some(UserDecision::OneTimeAccept)), &[]).expect("decide"))
                .unwrap();
