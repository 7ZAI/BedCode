...
impl Serialize for PairingCode {
        ...
        S: Serializer,
    {
        ...
        state.serialize_field("expires_in", &self.remaining_secs_at(crate::pairing::jwt::now_secs()))?;
        state.end()
    }
}
...
mod tests {
    ...
    fn ttl_boundary_elapsed_equals_ttl_not_expired() {
        ...
        assert!(!code.is_expired_at(1060), "elapsed == ttl 未过期（严格大于才过期）");
    ...
    fn remaining_secs_computes_residual() {
        ...
        assert_eq!(code.remaining_secs_at(1060), 0);
        assert_eq!(code.remaining_secs_at(2000), 0, "过期后剩余必须钳制 0，不能下溢");
    }
    ...
    fn rfc3339_roundtrip() {
        ...
        assert_eq!(parse_rfc3339_utc("2023-13-01T00:00:00Z"), None, "非法月份拒绝");
    }
}
