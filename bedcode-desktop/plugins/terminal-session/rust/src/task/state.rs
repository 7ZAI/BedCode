...
fn find_mapping_by_claude_sid(host: &WasmHost, claude_sid: &str) -> Option<String> {
    let result = host
        .plugin_db_query_params(
            "SELECT session_id FROM task_session_mapping WHERE claude_sid = ?1",
            &sql_params![claude_sid],
        )
        .ok()
        .flatten()?;
...
fn upsert_session_mapping(host: &WasmHost, claude_sid: &str, session_id: &str) {
    let _ = host.plugin_db_execute_params(
        "INSERT OR REPLACE INTO task_session_mapping (claude_sid, session_id, created_at) \
...
fn find_claude_sid_by_session(host: &WasmHost, bedcode_sid: &str) -> Option<String> {
    let result = host
        .plugin_db_query_params(
            "SELECT claude_sid FROM task_session_mapping WHERE session_id = ?1",
...
/// 注解槽键名——**contract**：内核 `task_fields_from_slot` 把这四个键机械映射到
/// 对外字段（`taskStatus` / `taskReason` / `taskUpdatedAt` / `taskQuestions`）。
/// 键名个数与拼写都不得漂移：漂一个字，移动端会话列表的任务字段就恒为空，而
/// 本域自己的 `task_history` 仍写着正确状态（最难排查的一类错）。
pub const SLOT_TASK_STATUS: &str = "taskStatus";
pub const SLOT_TASK_REASON: &str = "taskReason";
pub const SLOT_TASK_UPDATED_AT: &str = "taskUpdatedAt";
pub const SLOT_TASK_QUESTIONS: &str = "taskQuestions";

/// 任务状态快照 → 会话注解槽（对外会话 DTO 任务字段的唯一来源）
///
/// 槽只是**投影**，`task_history` 行才是本域真源：写槽失败（会话已销毁、宿主
/// annotation 表不可用）只 warn 降级，不阻断状态机推进（D7 单域降级）。
/// 空值一律以空串写入——内核读作「缺失」，对外字段即不出现。
pub fn publish_task_slots(
    host: &WasmHost,
    session_id: &str,
    status: &str,
    reason: Option<&str>,
...
/// 当前时刻（RFC3339 UTC）——WASM 无系统时钟，取宿主时钟或以 0 退化为纪元
fn now_rfc3339(host: &WasmHost) -> String {
    let secs = host
...
///
/// 同时反向查 task_session_mapping 写入 claude_sid：这样任务行同时带 claude_sid
...
pub fn handle_http_endpoint(
    ...
    match (method, path) {
        ...
        ("GET", "supported-agents") => {
            let agents = crate::task::agent::list_supported();
...
fn handle_update_task_status(host: &WasmHost, body: &Value) -> Value {
    ...
    // 解析 bedcode_session_id：显式携带优先；否则经 task_session_mapping 表把 claude_sid 解析成 bedcode sid
    ...
    if let Some(row) = existing {
        ...
        if is_current_terminal && !is_new_terminal {
            ...
            if status == "idle" {
                ...
                crate::task::queue::on_session_idle(host, &resolved_session_id);
                return http_response::ok();
            }
            host.log_info(&format!(
        ...
        // SessionStart 时还没有任务，映射关系在 task_session_mapping 表中维护
        if let Some(bedcode_sid) = bedcode_session_id.filter(|s| !s.is_empty()) {
            upsert_session_mapping(host, session_id, bedcode_sid);
            host.log_info(&format!(
...
fn handle_set_session_mode(host: &WasmHost, body: &Value, _query: &Value) -> Value {
    ...
    // hook/移动端可能携带 claude_sid，不经解析会把开关写到错误的 task_session_settings 行，
...
fn backfill_working_dirs(host: &WasmHost, mut rows: Vec<Value>) -> Vec<Value> {
    ...
    // 让出护栏：本循环是纯内存回填（历史行数量由调用方 limit 决定，可能上千），
    // 一次宿主调用都不发 → async store 内无 await 点，必须周期性让出（D7）
    ...
    let mut result: Vec<Value> = sessions
        ...
        .filter_map(|s| {
            ...
            let started_at = task
                ...
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            let queue_count = crate::task::queue::pending_count(host, &session_id);
            ...
            Some(serde_json::json!({
                ...
                "agent": agent,
                "is_supported": crate::task::agent::is_supported(agent),
...
pub fn set_auto_mode(
    ...
    if new_execute
        ...
        && !crate::task::queue::has_inflight_task(host, session_id)
    {
        crate::task::queue::try_dispatch_next(host, session_id);
    }
...
// ============ 会话级开关（task_session_settings 表） ============

/// 读取会话的 auto_execute（自动执行）与 auto_answer（自动应答）开关
///
/// task_session_settings 表为唯一事实来源；旧会话无该表记录时，
...
pub fn session_flags(host: &WasmHost, session_id: &str) -> (bool, bool) {
    let row = host
        .plugin_db_query_params(
            "SELECT auto_execute, auto_answer FROM task_session_settings WHERE session_id = ?1",
            &sql_params![session_id],
        )
        .ok()
        .flatten()
...
pub fn set_session_flags(
    ...
    // INSERT OR REPLACE：按 session_id 覆盖，与 task_session_mapping 的 upsert 模式一致
    let _ = host.plugin_db_execute_params(
        "INSERT OR REPLACE INTO task_session_settings (session_id, auto_execute, auto_answer, updated_at) \
