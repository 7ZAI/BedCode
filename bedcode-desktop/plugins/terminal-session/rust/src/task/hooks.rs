...
use crate::task::agent::{self, SessionIntegration};
...
pub fn cleanup_all_agent_integrations(host: &WasmHost) -> AllAgentIntegrationResult {
    ...
    //
    // 让出护栏：working_dir 为空的配置会整轮 `continue` 而不产生任何宿主调用，
    // 配置数量多时这条循环在 async store 内没有 await 点（D7：不得拖垮配对回调）
...
/// （端口匹配检查无法发现脚本内容更新）。
/// v4（票 17）：回调前缀改指 `com.bedcode.terminal-session`——随包脚本内容在票 16 已改，
/// 但**版本 bump 刻意压到 auto-task 后端退役之后**（双轨期两侧各自比对自己那份常量，
...
/// 模板版本标记：内容升级时递增模板内标记，旧部署副本据此自动重部署
/// （端口匹配检查无法发现脚本内容更新）
pub(crate) const CODEX_HOOK_TEMPLATE_VERSION: &str = "3";
