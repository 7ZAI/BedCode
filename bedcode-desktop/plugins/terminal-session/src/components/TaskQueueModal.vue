...
/**
 * TaskQueueModal — 任务队列弹窗（票 17 自 `com.bedcode.auto-task` 的 AutoTaskModal 搬入）
 *
...
const visible = taskModalVisible
...
function showError(message: string, e?: unknown) {
  ...
  if (e) console.error('[Session Center/task]', message, e)
}
...
async function loadQueue() {
  ...
  try {
    const result: any = await context.commands.execute('session.task.queue-list', {
      session_id: sessionId.value,
    })
    queue.value = (result?.tasks as QueueItem[]) || []
    activeTask.value = (result?.active_task as QueueItem | null) || null
  } catch (e) {
    console.error('[Session Center/task] Failed to load queue:', e)
    showError(t('task.loadFailed'))
  }
}
...
async function loadCurrentTask() {
  if (!sessionId.value) return
  try {
    const result: any = await context.commands.execute('session.task.history-list', {
      session_id: sessionId.value,
    })
    const rows = (result?.tasks as HistoryRecord[]) || []
    currentTask.value = rows[0] || null
  } catch (e) {
    console.error('[Session Center/task] Failed to load current task:', e)
    showError(t('task.loadFailed'))
  }
}
...
async function addPreset(presetId: string) {
  ...
  try {
    ...
    showError(t('task.addPresetFailed'), e)
  } finally {
...
async function loadSessionSettings() {
  if (!sessionId.value) return
  try {
    const result: any = await context.commands.execute('session.task.session-settings', {
      session_id: sessionId.value,
    })
    autoExecute.value = result?.auto_execute === true
    autoAnswer.value = result?.auto_answer === true
  } catch (e) {
    console.error('[Session Center/task] Failed to load session settings:', e)
    showError(t('task.loadFailed'))
  }
}
...
async function handleAdd() {
  ...
  try {
    ...
    console.error('[Session Center/task] Failed to add task:', e)
    showError(t('task.addFailed'), e)
  }
}
...
async function handleRemove(taskId: string) {
  if (!sessionId.value) return
  try {
    await context.commands.execute('session.task.queue-remove', {
      session_id: sessionId.value,
      task_id: taskId,
    })
    await loadQueue()
  } catch (e) {
    console.error('[Session Center/task] Failed to remove task:', e)
    showError(t('task.removeFailed'), e)
  }
}
...
async function handleCancelTask(taskId: string) {
  if (!sessionId.value) return
  try {
    await context.commands.execute('session.task.queue-cancel', {
      session_id: sessionId.value,
      task_id: taskId,
    })
    ...
    console.error('[Session Center/task] Failed to cancel task:', e)
    showError(t('task.cancelTaskFailed'), e)
  }
}
...
async function confirmClear() {
  ...
  try {
    await context.commands.execute('session.task.queue-clear', {
      session_id: sessionId.value,
    })
    queue.value = []
  } catch (e) {
    console.error('[Session Center/task] Failed to clear queue:', e)
    showError(t('task.clearFailed'), e)
  }
}
...
async function commitReorder(items: QueueItem[]) {
  if (!sessionId.value) return
  try {
    await context.commands.execute('session.task.queue-reorder', {
      session_id: sessionId.value,
    ...
    console.error('[Session Center/task] Failed to reorder queue:', e)
    showError(t('task.reorderFailed'), e)
    await loadQueue()
  }
}
...
async function saveEdit() {
  ...
  try {
    ...
    showError(t('task.updateFailed'), e)
  }
}
...
async function toggleAutoExecute() {
  ...
  try {
    await context.commands.execute('session.task.set-auto-mode', {
      session_id: sessionId.value,
      auto_execute: target,
    })
    ...
    console.error('[Session Center/task] Failed to toggle auto execute:', e)
    showError(t('task.modeFailed'), e)
  }
}
...
async function toggleAutoAnswer() {
  ...
  try {
    ...
    console.error('[Session Center/task] Failed to toggle auto answer:', e)
    showError(t('task.modeFailed'), e)
  }
}
...
<template>
  <div v-if="visible" class="at-overlay" @click.self="visible = false">
    <div class="at-modal">
      <!-- Header -->
      <div class="at-header">
        <h3 class="at-title">{{ t('task.title') }}</h3>
        <div class="at-header-actions">
          ...
          <button class="at-close" :title="t('task.close')" @click="visible = false">
            <svg
              width="16"
              height="16"
              viewBox="0 0 24 24"
      ...
      <div v-if="errorMessage" class="at-error" role="alert">
        ...
        <button class="at-error-close" :title="t('task.close')" @click="clearError">
          <svg
            width="12"
            height="12"
            viewBox="0 0 24 24"
      ...
      <div v-if="confirmingClear" class="at-confirm" role="alertdialog">
        <span class="at-confirm-text">{{ t('task.clearConfirm') }}</span>
        <div class="at-confirm-actions">
          <button class="at-btn at-btn-danger" @click="confirmClear">
            {{ t('task.confirm') }}
          </button>
          <button class="at-btn" @click="cancelClear">{{ t('task.cancel') }}</button>
        </div>
      </div>

      <!-- 无会话提示 -->
      <div v-if="!sessionId" class="at-empty">
        <p class="at-empty-text">{{ t('task.noSession') }}</p>
      </div>
      ...
      <template v-else>
        ...
        <div class="at-mode-row">
          <div>
            <p class="at-mode-label">{{ t('task.autoExecute') }}</p>
            <p class="at-mode-hint">{{ t('task.autoExecuteHint') }}</p>
          </div>
          <button
            role="switch"
        ...
        <div class="at-mode-row">
          <div>
            <p class="at-mode-label">{{ t('task.autoAnswer') }}</p>
            <p class="at-mode-hint">{{ t('task.autoAnswerHint') }}</p>
          </div>
          <button
            role="switch"
        ...
        <div v-if="presets.length > 0" class="at-presets">
          <p class="at-presets-title">{{ t('task.presetTitle') }} ({{ presets.length }})</p>
        ...
        <div v-if="activeTask" class="at-active-task">
          <span class="at-active-task-label">{{ t('task.activeTask') }}</span>
        ...
        <div class="at-queue">
          <div v-if="loading" class="at-empty">
            <p class="at-empty-text">{{ t('task.loading') }}</p>
          </div>
          <div v-else-if="queue.length === 0" class="at-empty">
            <p class="at-empty-text">{{ t('task.emptyQueue') }}</p>
            <p class="at-empty-hint">{{ t('task.emptyHint') }}</p>
          </div>
            ...
            <div v-for="(item, index) in queue" :key="item.id" class="at-item">
              ...
              <div class="at-item-actions">
                ...
                <template v-else>
                  ...
                  <button class="at-icon-btn" :title="t('task.edit')" @click="startEdit(item)">
                    <svg
                      width="14"
                      height="14"
                      viewBox="0 0 24 24"
