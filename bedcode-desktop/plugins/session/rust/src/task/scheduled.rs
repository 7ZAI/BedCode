//! 定时自动任务域（票 16 自 `com.bedcode.auto-task` 搬入，v6 / ADR 0003）
//!
//! 指定触发时刻、一次性执行的自动任务：携带会话配置与一组 prompt，触发时新建会话
//! 并依次执行。状态机与搬迁前逐字一致（四态 + 中间态恢复路径见下）。
//!
//! **搬迁口径（spec D1 / 票 16）**：
//! - 表名随域统一为 `task_scheduled`（旧 `scheduled_jobs`，D5）；改名由
...
//! ```
//!
//! 终态处置：missed / failed 为不可自动恢复的终态，用户可删除（清理）或 reset（改
//! 触发时间后重新调度）；executed 为执行档案，前端归入历史区段。
//!
//! 触发链路与常规自动任务共用队列调度：Created 事件到达后把 prompts 注入新会话队列
//! （source='scheduled'）并开启会话自动执行开关，首轮任务由 agent SessionStart 的
//! idle 推送驱动下发（TUI 型 agent 无 idle 信号时由 tick 的宽限兜底主动调度）。
//!
//! 时间基准：WASM 无系统时钟，所有时间比较使用宿主 tick 回调注入的 `now_utc`
//! （与 SQLite datetime('now') 同格式，字符串可直接比较）。
//!
...
pub fn create_job(
    ...
    match host.plugin_db_execute_params(
        "INSERT INTO task_scheduled (id, name, config_id, trigger_at, prompts, status, created_at) \
...
pub fn list_jobs(host: &WasmHost) -> Vec<Value> {
    host.plugin_db_query(
        "SELECT id, name, config_id, trigger_at, prompts, status, session_id, created_at, executed_at, error \
         FROM task_scheduled ORDER BY trigger_at ASC",
    )
    .ok()
    .flatten()
...
pub fn delete_job(host: &WasmHost, job_id: &str) -> bool {
    let affected = host
        .plugin_db_execute_params(
            "DELETE FROM task_scheduled WHERE id = ?1 AND status IN ('pending', 'missed', 'failed', 'executed')",
            &sql_params![job_id],
        )
        .unwrap_or(-1);
    if affected > 0 {
...
pub fn reset_job(host: &WasmHost, job_id: &str, trigger_at: Option<&str>) -> bool {
    let affected = host
        .plugin_db_execute_params(
            "UPDATE task_scheduled SET status = 'pending', \
...
///
/// 返回本域失败明细给 [`super::tick_via_host`]：私有库读写失败不再 `.unwrap_or(-1)`
/// 静默成「本轮没有到期任务」，而是让本域降级可见（同票 15 对队列域的处置）。
pub fn handle_scheduler_tick(host: &WasmHost, now_utc: &str) -> Result<(), String> {
    // 步骤内失败**按步登记、不短路后续步骤**（与搬迁前 `.unwrap_or(-1)` 的「所有步骤
    // 都执行」保持一致），但失败不再静默：整域返回 Err 交 tick 分发表按域计数（D7）。
    ...
    // 2. 到期且仍在宽限期内：触发执行
    let due_jobs = match host.plugin_db_query_params(
        "SELECT id, config_id, prompts FROM task_scheduled \
        ...
        match created {
            Ok(session_id) => {
                ...
                let _ = host.plugin_db_execute_params(
                    "UPDATE task_scheduled SET status = 'creating', session_id = ?1 WHERE id = ?2",
            ...
            Err(e) => {
                let _ = host.plugin_db_execute_params(
                    "UPDATE task_scheduled SET status = 'failed', executed_at = datetime('now'), error = ?1 \
                     WHERE id = ?2",
    ...
    let stale_sessions = match host.plugin_db_query_params(
        &format!(
            "SELECT DISTINCT q.session_id AS session_id FROM task_queue q \
             JOIN task_scheduled j ON j.session_id = q.session_id AND j.status = 'executed' \
    ...
    // 旧实现的第 4 步「executing 静默看门狗」不在此调用：合并插件把它登记成 tick
    // 分发表里的独立域（[`super::DOMAIN_QUEUE_SILENCE`]），单域失败只降级本域（D7）。
    // 一轮 tick 里它仍恰好执行一次，只是不再因定时域出错而被一起跳过。
...
pub fn handle_session_created(host: &WasmHost, session_id: &str, config_id: &str) {
    let job = host
        .plugin_db_query_params(
            "SELECT id, prompts FROM task_scheduled WHERE status = 'creating' AND session_id = ?1",
            &sql_params![session_id],
        )
        .ok()
        .flatten()
...
///
/// 应用退出会销毁全部 PTY 会话，creating 态等待的 Created 事件永远不会到达
/// （新进程的新会话不属于该任务），直接终结避免永久卡死。
///
/// 返回被终结的数量供调用方记日志（票 15 起 activate 统一在本模块外记录降级）；
/// 私有库不可用返回 Err，activate 侧只降级定时域（D7）。
pub fn recover_creating_jobs(host: &WasmHost) -> Result<i32, String> {
    let affected = host
        .plugin_db_execute_params(
            "UPDATE task_scheduled SET status = 'failed', executed_at = datetime('now'), \
        ...
        .map_err(|e| format!("recover_creating_jobs update failed: {}", e.message))?;
    if affected > 0 {
        broadcast_scheduled_changed(host, "", "failed", "failed");
    }
    Ok(affected)
