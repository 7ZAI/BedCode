...
//!
//! **dead_code 收窄（票 05 落地，票 06 定稿）**：policy 搬入后，`JwtClaims` /
//! `b64url_decode` / `is_expired_at` / `JWT_ISSUER` / `now_secs` 已有生产消费方；
//! 仍无生产消费方的是**签发面**（`JwtService` / `sign_hs256` / `b64url_encode` /
//! header / leeway / `DEFAULT_TOKEN_EXPIRY_SECS`）——生产签发永远留宿主
//! `src-tauri/src/utils/auth/jwt.rs`（spec D3「不动」列），本模块签发面只服务
//! 「与宿主 jsonwebtoken 同输入同输出」的对照测试。故由模块级 `#![allow(dead_code)]`
//! 收窄为**逐项** `#[allow(dead_code)]`：新增未使用项仍会被编译器看见。
//! 验签执行点始终留宿主中间件（spec D3「不动」列），本模块不参与生产验签。
