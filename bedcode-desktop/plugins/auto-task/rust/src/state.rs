...
fn handle_update_task_status(host: &WasmHost, body: &Value) -> Value {
    ...
    if !validate_token(host, token) {
        host.log_warn(&format!("task-status auth failed: session_id={}", session_id));
    ...
    if let Some(bedcode_sid) = bedcode_session_id {
        if !bedcode_sid.is_empty() {
            host.storage_set(&format!("{}{}", SESSION_MAP_PREFIX, session_id), &serde_json::json!(bedcode_sid));
    ...
    // 存储状态
    host.storage_set(&format!("{}{}", TASK_STATUS_PREFIX, storage_key), &serde_json::json!(status));
    if let Some(r) = reason {
        host.storage_set(&format!("{}{}", TASK_REASON_PREFIX, storage_key), &serde_json::json!(r));
    }
    if let Some(q) = questions {
        host.storage_set(&format!("{}{}", TASK_QUESTIONS_PREFIX, storage_key), q);
    }
...
fn handle_set_session_mode(host: &WasmHost, body: &Value, query: &Value) -> Value {
    ...
    if !token_valid {
        host.log_warn(&format!("session-mode POST auth failed: session_id={}", session_id));
        return error_response(403, "Invalid plugin token or JWT authentication");
    }

    // 存储模式
    host.storage_set(&format!("{}{}", AUTO_MODE_PREFIX, session_id), &serde_json::json!(auto_approve));
...
fn handle_get_session_mode(host: &WasmHost, query: &Value) -> Value {
    ...
    // 验证 plugin token
    if !validate_token(host, token) {
        host.log_warn(&format!("session-mode GET auth failed: session_id={}", session_id));
        return error_response(403, "Invalid plugin token");
    }
    ...
    // 查询自动模式
    let auto_approve = host.storage_get(&format!("{}{}", AUTO_MODE_PREFIX, &resolved_id))
...
fn validate_token(host: &WasmHost, token: &str) -> bool {
    ...
    !config_token.is_empty() && token == config_token
}
...
fn resolve_session_id(host: &WasmHost, claude_session_id: &str) -> String {
    host.storage_get(&format!("{}{}", SESSION_MAP_PREFIX, claude_session_id))
...
pub fn get_task_status(host: &WasmHost, session_id: &str) -> anyhow::Result<Value> {
    let status = host.storage_get(&format!("{}{}", TASK_STATUS_PREFIX, session_id));
    Ok(serde_json::json!({
        "session_id": session_id,
        "task_status": status,
    }))
