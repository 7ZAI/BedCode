...
const peerDisplayName = computed(() => {
  ...
  return peer.value.ip || peer.value.id || t('transfer.peer.unpaired')
})

const selectedCount = computed(() => selectedNames.value.length)

/** 主下载按钮可用性：有选择 + 对端在线 + 已配下载目录 */
...
const showNoPeer = computed(() => peer.value.online === false)
  ...
  <div class="ft-view">
    ...
    <div class="ft-topbar">
      <div class="ft-peer-pill">
        <span class="ft-dot" :class="peer.online ? 'ft-dot--online' : 'ft-dot--offline'"></span>
        <span class="ft-peer-name">{{ peerDisplayName }}</span>
        <span class="ft-peer-meta">
          {{ peer.online ? t('transfer.peer.online') : t('transfer.peer.offline') }}
        </span>
      </div>
    ...
    <div v-else-if="showNoPeer" class="ft-empty">
      <div class="ft-empty-ico">📱</div>
      <div>{{ t('transfer.empty.noPeer') }}</div>
    </div>
