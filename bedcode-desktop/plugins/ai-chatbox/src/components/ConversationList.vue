    ...
    <template v-else>
      ...
      <div class="flex-1 overflow-y-auto py-1 flex flex-col">
        <div
          ...
          :class="conv.id === currentId ? 'bg-[var(--bg-hover)]' : 'hover:bg-[var(--bg-hover)]'"
        ...
        >
          ...
          <!-- 操作（悬停显示） -->
          <span
            class="opacity-0 group-hover:opacity-100 transition-opacity flex items-center flex-shrink-0"
          >
            <button
        ...
        >
          <div
            class="w-12 h-12 rounded-card bg-[var(--bg-hover)] flex items-center justify-center mb-3 text-[var(--text-tertiary)]"
          >
            <svg
              class="w-6 h-6"
              fill="none"
