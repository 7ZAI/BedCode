    ...
    <Transition name="page-fade" mode="out-in">
      ...
      <div v-else key="chat" class="flex w-full min-w-0">
        ...
        <div class="flex-1 flex flex-col min-w-0">
          <!-- 头部：对话标题 + 供应商/模型 + 设置 -->
          <header
            class="px-4 py-2 flex items-center justify-between border-b border-[var(--border)] bg-[var(--bg-card)]"
          >
          ...
          >
            <div
              class="w-14 h-14 rounded-card bg-[var(--bg-card)] border border-[var(--border)] flex items-center justify-center mb-4 text-[var(--text-secondary)]"
            >
              <svg
                class="w-7 h-7"
                fill="none"
          ...
          <template v-else>
            ...
            <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              >
                <div
                  class="w-12 h-12 rounded-full bg-[var(--bg-hover)] flex items-center justify-center mb-3 text-[var(--text-tertiary)]"
                >
                  <svg
                    class="w-6 h-6"
                    fill="none"
...
const currentModelKey = computed(() =>
  activeProviderId.value && activeModel.value
    ? modelKey(activeProviderId.value, activeModel.value)
    : '',
)
