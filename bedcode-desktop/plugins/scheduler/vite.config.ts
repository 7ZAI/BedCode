  ...
  return {
    ...
    generateBundle(_options, bundle) {
      ...
      for (const fileName of Object.keys(bundle)) {
        ...
        const code = `(function(){const id='data-plugin-css';const old=document.querySelectorAll('style['+id+']');old.forEach((el)=>el.remove());const style=document.createElement('style');style.setAttribute(id,'');style.textContent=${JSON.stringify(
          text,
        )};document.head.appendChild(style)})()`
        // 把 CSS 注入入口 chunk 顶部（依赖先于视图渲染执行）
        entry.code = code + '\n' + entry.code
      }
    },
  }
...
export default defineConfig({
  plugins: [vue(), inlinePluginCss(), bedcodePlugin()],
  build: {
    lib: {
