...
fn test_session_plugin_artifact_lifecycle() {
    ...
    assert_eq!(
        ...
        serde_json::json!([
            ...
            // → 前端权限门会抛错）
            "ui:sidebar",
            // 票 09b：`ws:server`（声明端点 session/control 的 events-ws 回包
            // 经 host-websocket send-text-to-client，服务端域判据位）
            "ws:server"
        ]),
...
/// 票 09b/09c：WS 会话控制声明式路由转发闭环（真实 wasm 产物 + 真实 host-pty）
///
/// 驱动宿主 `services::session_control::handle_control_message`（`/ws/event`
/// 的 SessionControl 分派入口）：声明闸门 → 原始动作 JSON 转发插件 `ws_control`
/// 域 → 响应动作 JSON 套回 `Message::SessionControl` 信封。断言：
/// - 声明闸门：端点未注册 → 显性 Err（不静默降级，未声明 = 宿主不解动作）；
/// - list：空登记域 → `SessionList { sessions: [] }`；
/// - start：播种配置 → 真 spawn（bash）→ 回执信封 session_id = 新会话 id、
///   动作回显 config_id（与旧宿主 wire 逐字一致）；
/// - list 再查：新会话 running + config_id 透传（SessionSummary 形状）；
/// - stop：回执 StopSession + 信封 session_id = 目标会话。
#[test]
fn test_ws_session_control_relay_closed_loop() {
    use crate::enums::{SessionControlAction, SessionControlPayload};
    use crate::server::websocket::message::Message as WsMessage;
    use crate::server::websocket::services::session_control;

    // 会话插件私有库是进程级共享路径：与其它会话闭环用例串行（见锁文档）
    let _serial = session_plugin_db_guard();
    // 声明闸门用例（services/session_control.rs）与本用例共用全局端点表：
    // 同一把锁串行化（注册/摘除 session/control 互不打断）
    let _endpoint_guard = session_control::SESSION_CONTROL_ENDPOINT_TEST_LOCK.lock().unwrap();
    ...
    // 与插件 manifest 权限表同源的最小集（activate 各域建表 + 会话面 host-pty +
    // 票 09b 声明端点的回包判据位 ws:server）
    host_ctx.permission.grant_permissions(
        SESSION_ID,
        &[
            "auth".to_string(),
    ...
    rt.block_on(async {
        ...
        let msg_id = |n: u32| format!("relay-req-{n}");

        // ==================== 1. 声明闸门反例：端点未注册 → 显性 Err ====================
        // 裸 instantiate+activate 不走 PluginHost::activate_plugin 的
        // register_declared_ws_endpoints，此时端点表无 session/control →
        // 转发必须显性拒绝（未声明 = 宿主对该动作无解释义务，不静默降级）
        let gate_err = session_control::handle_control_message(
            &host_ctx,
            msg_id(0),
            None,
            0,
        ...
        .await
        .expect_err("未声明端点必须拒绝转发");
        assert!(
            gate_err.to_string().contains("not declared/active"),
            "闸门文案须点明声明缺失, got: {gate_err}"
        );

        // ==================== 2. 激活期等价登记（PluginHost 激活成功分支的对应物） ====================
        ...
        // ==================== 3. list（空登记域）→ SessionList [] ====================
        let seeded = seed_config_in_plugin_store(&mut *session.lock().await, "WS控制会话", "/tmp", "bash").await;
        let config_id = seeded["id"].as_str().expect("config id").to_string();

        let resp = session_control::handle_control_message(
            &host_ctx,
            msg_id(1),
            None,
            0,
        ...
        .await
        .expect("list relay");
        match resp.expect("list 有回包") {
            WsMessage::SessionControl { message_id, session_id, payload, .. } => {
                assert_eq!(message_id, "relay-req-1");
                assert!(session_id.is_none(), "list 信封不带 session_id（旧 wire 同形）");
                assert!(
                    matches!(payload.action, SessionControlAction::SessionList { ref sessions } if sessions.is_empty()),
                    "空登记域 → 空列表, got: {:?}",
                    payload.action
                );
            }
        ...
        .await
        .expect("start relay");
        let sid = match resp.expect("start 有回包") {
            WsMessage::SessionControl { message_id, session_id, payload, .. } => {
                assert_eq!(message_id, "relay-req-2");
                let sid = session_id.expect("start 信封必须带新会话 id（移动端据此拿新会话）");
                match payload.action {
                    SessionControlAction::StartSession { config_id: act_cfg } => {
                        assert_eq!(act_cfg, config_id, "start 动作回显 config_id（旧 wire 逐字一致）");
                    }
        ...
        assert_eq!(sid.len(), 36, "插件自产 UUID");

        // ==================== 5. list 再查：新会话 running + SessionSummary 形状 ====================
        let resp = session_control::handle_control_message(
            &host_ctx,
            msg_id(3),
            None,
        ...
        .expect("list relay 2");
        match resp.expect("list 有回包") {
            WsMessage::SessionControl { payload: SessionControlPayload { action }, .. } => {
                match action {
                    SessionControlAction::SessionList { sessions } => {
                        let entry = sessions
                            .iter()
                            .find(|s| s.id == sid)
                            .expect("新会话必须出现在列表");
                        assert_eq!(entry.status, "running", "创建即启动 → running");
                        assert_eq!(
                            entry.config_id.as_deref(),
                            Some(config_id.as_str()),
                            "config_id 透传（SessionSummary 形状与旧 wire 逐字一致）"
                        );
                    }
        ...
        // ==================== 6. stop → 回执 StopSession + 信封 session_id ====================
        let resp = session_control::handle_control_message(
            &host_ctx,
            msg_id(4),
            None,
            0,
            SessionControlAction::StopSession { session_id: sid.clone() },
            addr,
            None,
            None,
        )
        .await
        .expect("stop relay");
        match resp.expect("stop 有回包") {
            WsMessage::SessionControl { message_id, session_id, payload, .. } => {
                assert_eq!(message_id, "relay-req-4");
                assert_eq!(session_id.as_deref(), Some(sid.as_str()), "stop 信封回显目标会话");
                match &payload.action {
                    SessionControlAction::StopSession { session_id: echoed } => {
                        assert_eq!(echoed, &sid, "stop 动作回显 session_id");
                    }
                    other => panic!("stop 动作回显 session_id, got: {other:?}"),
                }
            }
            other => panic!("expected SessionControl, got: {other:?}"),
        }
        ...
        // ==================== 清理 ====================
        let _ = crate::utils::session_gateway::remove(&host_ctx, &sid, None).await;
        crate::server::websocket::endpoint::purge_for_plugin(SESSION_ID);
        session.lock().await.deactivate().expect("final deactivate");
    });
}
