...
impl PluginHost {
    ...
    pub async fn approve_plugin(&self, plugin_id: &str) -> crate::Result<Vec<String>> {
        ...
        let approved = crate::wasm_core::security::approval::known_permissions(&requested);
        // 目录哈希不进 async 事件循环（插件目录可能较大，避免阻塞 runtime worker）
        let dir = PathBuf::from(&extension_path);
        let content_hash =
            tokio::task::spawn_blocking(move || crate::wasm_core::security::approval::compute_dir_hash(&dir))
                .await
                .map_err(|e| crate::AppError::Plugin(format!("Approval hash task failed: {}", e)))??;

        crate::wasm_core::security::approval::PluginApprovalStore::new(self.storage.clone())
    ...
    pub async fn uninstall_plugin(&self, plugin_id: &str) -> crate::Result<()> {
        ...
        if let Err(e) = crate::wasm_core::security::approval::PluginApprovalStore::new(self.storage.clone())
            .revoke(plugin_id)
            .await
        {
            tracing::warn!(
