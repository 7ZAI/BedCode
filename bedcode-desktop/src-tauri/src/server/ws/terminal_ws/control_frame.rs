...
pub enum WatchMode {
    /// 实时：进终端页实时显示——维持读即传（时间窗+字节窗合并）
    Realtime,
    /// 批次：退出终端页但会话未停——满 batch_bytes 才转发一帧（可配置）
    Batch,
}
...
pub enum ClientFrame {
    ...
    /// 订阅绑定会话（快照协议）：from_offset 可选 = 客户端已渲染字节游标，
    /// 历史按该锚点截取回放（TB v3 快照截取）；缺省全量回放（老客户端兼容）
    Subscribe {
        #[serde(default)]
    ...
    /// 切换实时传播模式（双速，用户需求 3）：realtime（进终端页，读即传）/
    /// batch（退出终端页但会话未停，满 terminal.batch_bytes 才转发）
    #[serde(rename = "mode")]
...
pub enum ServerFrame {
    ...
    /// 订阅已建立（快照元数据，字节三件套；历史帧在其后按序到达）
    SubscribeOk {
        /// 协议版本（3 = TB v3 字节偏移语义；老客户端忽略）
        protocol: u8,
        /// 订阅时刻累计字节数（历史边界；历史帧 end_offset ≤ 该值）
        snapshot_offset: u64,
        /// 队列最早存续字节位置（环形淘汰后推进；> 客户端游标 → 已截断）
        min_offset: u64,
        /// 驻留历史总字节数
        history_bytes: u64,
    },
...
// 复用 TB 帧头（magic(2) + version(1) + flags(1) + offset(8 LE) + len(4 LE) = 16
// 字节）：客户端→服务端方向，flags 置 ACK 标志位，offset = 已渲染到的字节游标
//（last_rendered_offset = end_offset），len + payload = session_id UTF-8 字节。
// 与服务端→客户端输出帧的 WAITING 位语义互不冲突（该方向不解析 ack）。
// 版本：TB v3 主版本；v2 旧客户端 ack（seq 语义）同样接受——弹出条件
//「end_offset ≤ 游标」对 seq 值只可能少弹不越减，安全方向兼容

/// TB 帧头长度
pub const TB_FRAME_HEADER_LEN: usize = 16;
const TB_FRAME_MAGIC: [u8; 2] = [0x54, 0x42]; // "TB"
/// 输出帧协议版本（TB v3：字节偏移语义）
const TB_FRAME_VERSION: u8 = 3;
/// 旧客户端 ack 版本（TB v2 seq 语义，兼容接受）
const TB_FRAME_VERSION_V2: u8 = 2;
/// 背压 ack 标志位（仅客户端→服务端使用）
const TB_FRAME_FLAG_ACK: u8 = 0x02;

/// 解析客户端背压 ack 帧，返回 `(acked_offset, session_id)`
///
...
pub fn parse_ack_frame(bytes: &[u8]) -> Result<(u64, String), ()> {
    ...
    if bytes[0] != TB_FRAME_MAGIC[0]
        || bytes[1] != TB_FRAME_MAGIC[1]
        || (bytes[2] != TB_FRAME_VERSION && bytes[2] != TB_FRAME_VERSION_V2)
    {
        return Err(());
    }
    ...
    let acked_offset = u64::from_le_bytes(bytes[4..12].try_into().map_err(|_| ())?);
...
mod tests {
    ...
    fn parse_subscribe_frame() {
        // 无 from_offset（老客户端）：全量回放
        let frame = parse_client_frame(r#"{"type":"subscribe"}"#).unwrap();
        assert!(matches!(frame, ClientFrame::Subscribe { from_offset: None }));
    ...
    fn parse_set_mode_frame() {
        ...
        assert!(matches!(frame, ClientFrame::SetMode { mode: WatchMode::Batch }));

        // 非法模式被拒
        assert!(parse_client_frame(r#"{"type":"mode","mode":"turbo"}"#).is_err());
    }
    ...
    fn serialize_history_end() {
        let json = ServerFrame::HistoryEnd { snapshot_offset: 7 }.to_json();
        let v: serde_json::Value = serde_json::from_str(&json).unwrap();
        assert_eq!(v["type"], "history_end");
        assert_eq!(v["snapshot_offset"], 7);
    }
    ...
    /// 构造客户端背压 ack 帧（与前端 buildAckFrame 逐字节对齐，TB v3）
    fn build_ack(session_id: &str, acked_offset: u64) -> Vec<u8> {
    ...
    fn parse_ack_frame_valid() {
        ...
        // v2 旧客户端 ack（seq 语义同值域）兼容接受
        let mut v2 = build_ack("s1", 7);
        v2[2] = TB_FRAME_VERSION_V2;
        assert_eq!(parse_ack_frame(&v2).unwrap(), (7, "s1".to_string()));
    }
    ...
    fn parse_ack_frame_roundtrip_offsets() {
        // 前端帧头布局逐字节断言（magic/version/flags/offset(8 LE)/len(4 LE)）
