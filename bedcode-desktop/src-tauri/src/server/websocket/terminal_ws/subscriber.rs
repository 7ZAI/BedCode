...
/// 的输出订阅执行体——语义与内核环 [`subscriber_loop`] 逐字一致（subscribe_ok /
...
///
/// 返回形状与 [`spawn_subscriber`] 逐字一致（subscribe_ok 三件套 + 执行体 + 交接
/// 通道），调用方无需区分数据源；差异只在内部读源（引擎环 vs 内核环）。
...
/// [`subscriber_loop`] 逐字一致（subscribe_ok / history_end / resync / TB v3），
/// 差异只在读源与唤醒机制：
