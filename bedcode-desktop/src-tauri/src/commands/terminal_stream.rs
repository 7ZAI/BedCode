...
pub async fn subscribe_terminal_channel(
    ...
    let response = match manager.subscribe(&session_id, &client_id, output_tx, None).await {
        Some(r) => r,
        None => {
            // 会话不存在：已 spawn 的 forward/consumer 任务永远等不到数据，
            // 显式 abort 避免任务泄漏（否则两个挂起的 await 常驻）
            fwd_handle.abort();
