...
fn ensure_unit_path_granted(host_ctx: &Arc<WasmHostContext>, owner: &str, unit: &PlanUnit) -> Result<(), String> {
    ...
    let permission = if needs_write {
        crate::plugin::permission::PERMISSION_FS_WRITE
    } else {
        crate::plugin::permission::PERMISSION_FS_READ
    };
    ...
    if crate::plugin::manager::wasm_runtime::block_on_async(host_ctx.fs_auth().is_granted(owner, path)) {
        return Ok(());
    }
    Err(format!(
...
mod tests {
    ...
    fn fs_unit_gate_follows_pipeline_order_and_never_prompts() {
        use crate::plugin::manager::wasm_runtime::host_impl::tests::build_host_ctx;
        ...
        crate::plugin::manager::wasm_runtime::block_on_async(ctx.fs_auth().save_granted_path("com.test.p", &path))
            .expect("seed grant");
