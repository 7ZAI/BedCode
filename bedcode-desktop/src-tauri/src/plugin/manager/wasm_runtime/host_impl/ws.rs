...
pub(crate) fn ws_connect(host_ctx: &WasmHostContext, plugin_id: &str, config_json: &str) -> Result<String, String> {
    ...
    let (stream, response) = crate::plugin::manager::wasm_runtime::block_on_async(async move {
        tokio::time::timeout(
...
pub(crate) fn ws_register_endpoint(
    ...
    if path.chars().count() > crate::system::constants::plugin::PLUGIN_WS_ENDPOINT_PATH_MAX_LEN {
        return Err(format!(
...
pub(crate) fn ws_broadcast_text(
    ...
    let endpoint = endpoint_id.to_string();
    let text = text.to_string();
    let sent = crate::plugin::manager::wasm_runtime::block_on_async(async move {
...
pub(crate) fn ws_close_client(
    ...
    let endpoint = endpoint_id.to_string();
    let client = client_id.to_string();
    Ok(crate::plugin::manager::wasm_runtime::block_on_async(async move {
        WsSessionRegistry::global()
...
pub(crate) fn ws_list_clients(
    ...
    owned_endpoint(endpoint_id, plugin_id)?;
    let endpoint = endpoint_id.to_string();
    let clients = crate::plugin::manager::wasm_runtime::block_on_async(async move {
...
pub(crate) fn ws_list_endpoints(host_ctx: &WasmHostContext, plugin_id: &str) -> Result<String, String> {
    ...
    for entry in entries {
        let endpoint = entry.endpoint_id.clone();
        let client_count = crate::plugin::manager::wasm_runtime::block_on_async(async move {
...
fn max_message_bytes() -> usize {
    crate::server::app::ws_frame_limit().max(PLUGIN_WS_MAX_MESSAGE_BYTES.min(1))
}
...
mod tests {
    ...
    async fn register_endpoint_validates_shape_and_auth() {
        ...
        assert!(ws_register_endpoint(&ctx, &plugin, r#"{"path":".."}"#)
            .unwrap_err()
            .contains("must not contain"));
        let too_long = "x".repeat(crate::system::constants::plugin::PLUGIN_WS_ENDPOINT_PATH_MAX_LEN + 1);
        assert!(
    ...
    async fn register_endpoint_conflict_and_limit_are_side_effect_free() {
        ...
        grant_permissions(&ctx, &plugin, &[PERMISSION_WS_SERVER]);
        let limit = crate::system::constants::plugin::PLUGIN_WS_MAX_ENDPOINTS_PER_PLUGIN;
    ...
    impl crate::plugin::bus::BusMessageHandler for TopicSink {
