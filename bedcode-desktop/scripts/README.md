...
5. **HTTP Push**: 任务状态变更推送到 `POST /api/plugin/com.bedcode.terminal-session/task-status`（旧 `com.bedcode.auto-task` 前缀由宿主别名表继续应答，见 `docs/knowledge/plugin-http-endpoint-trust.md`）。端点由网关中间件对 `/api/plugin/*` 路径本地放行（无凭证），hook 仅在 BedCode 注入 `BEDCODE_SESSION_ID` 的 PTY 中生效，信任边界为本地网络
