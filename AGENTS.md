...
- [ ] **会话真源在插件（P1-b，2026-09-24）**：会话登记 / 状态机 / 生命周期分发 / 输入输出编排归
  ...
  server 连接事实，**票 04 已迁独立原语 `host-connection.connections-list`**，此处只留同判据别名待票 10 删；
  判据也从 `session:read` 换挂新位 **`connection:read`**——只授 `session:read` 不再能读连接清单），
  `host-terminal.terminal_send` 同样零消费者。
