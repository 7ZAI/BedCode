...
**Status:** in-progress（2026-09-24 首切片：SDK 声明面 + 校验 + 宿主激活期登记已完成，见下）
...
- [ ] 将存量 WS 动作（会话启动/停止/删除/调整大小）**接入**声明式路由并验证帧时序（仍在旧 switch 路径）
- [ ] 无权限位 / 未声明插件对应动作不可达的集成测试补全
- [ ] 验收清单其余项随 09b / 09c 收口

**仍待（未动）**：声明式路由与既有硬编码 switch 的**并存运行**（集成测试断言零回退，09b）；
宿主无业务词表 switch（09c）
