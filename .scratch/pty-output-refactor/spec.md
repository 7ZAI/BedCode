...
| 项 | 策略 |
|----|------|
| 旧移动端（v2.0.0）↔ 新桌面端 | **保留旧 `/ws/terminal` 路由及 WS 认证各阶段**（RequestPairing/VerifyCode/QrConnect 等分支保留，标记 compat）。低成本、改动独立。长期观察后删除 |
...
| D2 | 旧客户端兼容 | 旧 `/ws/terminal` 路由 + WS 认证各阶段保留（compat 标记） | 成本低；删除需双端确认 |
...
| D2 | 旧客户端兼容 | 旧 `/ws/terminal` 路由 + WS 认证各阶段保留，标记 compat（ticket 11 记录观察期） |
