...
**Status:** in-progress（2026-09-23 二次开工：**peer-net 半票（验收 1/2/6 的主体）代码已写完**，
ABI 取 v25（对侧 v24 已提交于 `727041862`），SDK 侧 `cargo test --lib` 114/0 绿、
我自己那 5 个编译错已清；**但全量门禁跑不了、未提交** —— 第三个会话此刻正在做
`server/` 三层重命名且中间态不自洽（`server/websocket/*` 引用 `server::services` 共 5 个 E0433，
`websocket/conn.rs` 最后写盘 05:11，`cargo-clippy` 在跑）。
改动快照与收尾清单在 `pending/`（patch 对纯净 HEAD dry-run 零 reject）。
本轮另落了验收 4（preauth 文案，`b182dd1a1`）与验收 5（会话依赖登记 roadmap）；
**验收 3（四个迁移模块摘出 `plugin/`）仍未做**，理由见「开工复核」第 2 条。
上一版的 blocked 判据仍然有效，其中「ABI 被占用」一条已随 v24 提交而解除）
