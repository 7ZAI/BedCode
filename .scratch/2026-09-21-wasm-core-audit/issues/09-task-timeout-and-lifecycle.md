...
**Status:** done（2026-09-22：验收 2 本就存在、3/4/5/6 完成于 `28cf5e889` + `a2b169a0b`；验收 1 经用户裁决走**选项 B（退役不可兑现的单元超时常量并如实写文档）**，落地于本节最后一次提交。残余风险转入待立项 E5'）
...
- [x] ~~单元超时生效~~ → **按裁决 B 退役**：常量删除 + 文档写明「v20 无单元级抢占」（见 §D）；残余风险转入待立项 E5'
...
### D. 单元超时（验收 1）——**用户裁决 = 选项 B，已落地**

裁决（2026-09-22）：不实现单元级抢占，退役不可兑现的常量并如实写文档。落地：

- 删除 `PLUGIN_TASK_UNIT_TIMEOUT_MS`（`system/constants/plugin.rs`）——全仓零引用且无法兑现。
- `task.rs` 模块头新增一条口径：**v20 不提供单元级抢占**（单元执行体是同步阻塞直调，池线程内
  无中断点），兜底只有任务墙钟 `PLUGIN_TASK_JOB_TIMEOUT_MS`；阻塞型单元（`process.run-sync`）的
  超时由被调用方自带参数负责。
- SDK `host/task.rs` 的 `execute_batch` 阻塞上限说明同步改口径（原写「单元超时上限（宿主常量）」，
  常量已不存在）。
- 审计侧文档同步：`spec.md`（两处）标为已处置、`microkernel-gap.md` 的 E5 段写明「残余风险转入
  待立项 E5'（宿主调用侧统一超时闸门）」、`2026-09-21-host-task-concurrency/spec.md` 的常量表与
  重入纪律第 4 条同步。

**残余风险（不隐瞒）**：挂死单元仍会长期占用进程级 8 线程池槽位且跨插件无隔离——这正是 E5' 要
解决的「宿主调用侧统一超时闸门」，本票未做。

### D-2. 未做（原 §D 已按裁决关闭；保留备查）
