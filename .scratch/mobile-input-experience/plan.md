...
> 状态: 计划 (未排期)
> 已实施: P0-2 `/` 补全、P0-4 @ 文件引用插入（移动端仅，延续 ADR-0014 边界）；P0-1 已取消（见正文原因）
...
- 原因: 无法准确感知会话输出何时真正结束——桌面端 waitingInput 检测与插件 taskStatus
  均为预留未接入，靠「末行提示符 + 近期无输出」推断不可靠（CLI 提问等待输入、输出
  停顿、多行输出收尾等场景会误判），发送键被误切成中断按钮时用户无法发送输入。
- 决定: 保持发送键常驻；中断仍走快捷键面板 Esc / 快捷面板（原有通道）
- 涉及: `TerminalInputBar.vue`、`TerminalView.vue`（idle 轮询与 `isSessionBusy` 已移除）
- 备注: 待桌面端 waitingInput / taskStatus 真正接入后再评估恢复
