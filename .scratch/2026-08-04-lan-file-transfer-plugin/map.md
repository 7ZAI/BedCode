...
**v2.1 实施完成**（2026-08，双 subagent 并行 ①-③ + 主 agent ④ + list 迁移）：手机 client 四模块（download/upload/cursor）+ intent wire（FileTransferIntent/Cancel/IntentAck/进度心跳/fail）+ responder 状态机 + 审批落地（仅上传需落盘方审批）+ pull 批自批准 + 桌面协调者化（乐观更新/心跳）；**list 迁移**（施工图缺口，评审 13 预演）：桌面 `list-remote` 不再直连手机 `/list`，改经 `FileServicePayload::FileListRequest/Response` WS 往返（新 host fn `filesrv_list_remote` 同步等待 5s，前端零改动），手机 announce 改 port=0/token="" 纯挂载公告；**阶段④ 完成**：删除手机 `server.rs`/`auth.rs`（actix 整链移 dev-deps）、`tests/file_server_http.rs`、挂载生命周期改纯公告编排。两端 wire 逐字一致（snake_case）；mobile 392 lib + 集成全绿、desktop lib 570 全绿、插件 11 + SDK check 全绿、gradlew NOT re-run（无 Kotlin 改动）。**剩余**：真机联调（§8.3 清单）+ 手机 server 直连死代码清理（handshake.rs 旧兼容场底分支）。
